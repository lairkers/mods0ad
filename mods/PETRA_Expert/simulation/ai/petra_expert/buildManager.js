...
PETRA_EXPERT.BuildManager = function()
{
...
PETRA_EXPERT.BuildManager.prototype.init = function(gameState)
{
...
PETRA_EXPERT.BuildManager.prototype.incrementBuilderCounters = function(civ, ent, increment)
{
	for (let buildable of ent.buildableEntities(civ))
	{
		if (this.builderCounters.has(buildable))
		{
			...
			if (count < 0)
			{
				API3.warn(" Petra Expert error in incrementBuilderCounters for " + buildable + " with count < 0");
				continue;
			}
			...
			API3.warn(" Petra Expert error in incrementBuilderCounters for " + buildable + " not yet set");
	}
};

/** Update the builders counters */
PETRA_EXPERT.BuildManager.prototype.checkEvents = function(gameState, events)
{
 ...
 */
PETRA_EXPERT.BuildManager.prototype.findStructuresByFilter = function(gameState, filter)
{
	const result = [];
 ...
 */
PETRA_EXPERT.BuildManager.prototype.findStructureWithClass = function(gameState, classes)
{
	return this.findStructuresByFilter(gameState, API3.Filters.byClasses(classes))[0];
};

PETRA_EXPERT.BuildManager.prototype.hasBuilder = function(template)
{
	let numBuilders = this.builderCounters.get(template);
	return numBuilders && numBuilders > 0;
};

PETRA_EXPERT.BuildManager.prototype.isUnbuildable = function(gameState, template)
{
	return this.unbuildables.has(template) && this.unbuildables.get(template).time > gameState.ai.elapsedTime;
};

PETRA_EXPERT.BuildManager.prototype.setBuildable = function(template)
{
...
PETRA_EXPERT.BuildManager.prototype.setUnbuildable = function(gameState, template, time = 20, reason = "room")
{
	if (!this.unbuildables.has(template))
...
PETRA_EXPERT.BuildManager.prototype.numberMissingRoom = function(gameState)
{
	let num = 0;
...
PETRA_EXPERT.BuildManager.prototype.resetMissingRoom = function(gameState)
{
...
PETRA_EXPERT.BuildManager.prototype.Serialize = function()
{
	return {
...
PETRA_EXPERT.BuildManager.prototype.Deserialize = function(data)
{
	for (let key in data)
		this[key] = data[key];
};
