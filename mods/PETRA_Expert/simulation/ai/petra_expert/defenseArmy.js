 ...
 */
PETRA_EXPERT.DefenseArmy = function(gameState, foeEntities, type)
{
 ...
 */
PETRA_EXPERT.DefenseArmy.prototype.addFoe = function(gameState, enemyId, force)
{
 ...
 */
PETRA_EXPERT.DefenseArmy.prototype.removeFoe = function(gameState, enemyId, enemyEntity)
{
 ...
 */
PETRA_EXPERT.DefenseArmy.prototype.addOwn = function(gameState, id, force)
{
	...
	if (subrole === undefined || subrole !== PETRA_EXPERT.Worker.SUBROLE_DEFENDER)
		ent.setMetadata(PlayerID, "formerSubrole", subrole);
	ent.setMetadata(PlayerID, "subrole", PETRA_EXPERT.Worker.SUBROLE_DEFENDER);
	return true;
};

PETRA_EXPERT.DefenseArmy.prototype.removeOwn = function(gameState, id, Entity)
{
	...
	// TEST
	let formerSubrole = ent.getMetadata(PlayerID, "formerSubrole");
	if (formerSubrole === undefined || formerSubrole === PETRA_EXPERT.Worker.SUBROLE_DEFENDER)
    	ent.setMetadata(PlayerID, "subrole", PETRA_EXPERT.Worker.SUBROLE_IDLE);
	else
    	ent.setMetadata(PlayerID, "subrole", formerSubrole);
	ent.setMetadata(PlayerID, "formerSubrole", undefined);
	...
	if (ent.getMetadata(PlayerID, "transport") !== undefined)
	{
		...
		if (plan && plan.state === PETRA_EXPERT.TransportPlan.BOARDING && ent.position())
 ...
 */
PETRA_EXPERT.DefenseArmy.prototype.clear = function(gameState)
{
	...
	else
	{
		...
		for (let struct of gameState.getAllyStructures().values())
		{
			...
			if (!pos || !gameState.isPlayerMutualAlly(gameState.ai.HQ.territoryMap.getOwner(pos)))
				continue;
			if (PETRA_EXPERT.getLandAccess(gameState, struct) !== armyAccess)
				continue;
...
PETRA_EXPERT.DefenseArmy.prototype.assignUnit = function(gameState, entID)
{
	...
	for (let id of this.foeEntities)
	{
		...
		if (!ent.canAttackTarget(eEnt, PETRA_EXPERT.allowCapture(gameState, ent, eEnt)))
			continue;
		...
		if (this.assignedAgainst[id].length > 5 ||
			this.assignedAgainst[id].length > 2 && !eEnt.hasClass("Hero") && !PETRA_EXPERT.isSiegeUnit(eEnt))
			continue;
	...
	let ownIndex = PETRA_EXPERT.getLandAccess(gameState, ent);
	...
	if (ownIndex == foeIndex || ent.hasClass("Ship"))
	{
		...
		ent.attack(idFoe, PETRA_EXPERT.allowCapture(gameState, ent, foeEnt), queued);
	}
	else
...
PETRA_EXPERT.DefenseArmy.prototype.getType = function()
{
	return this.type;
};

PETRA_EXPERT.DefenseArmy.prototype.getState = function()
{
 ...
 */
PETRA_EXPERT.DefenseArmy.prototype.merge = function(gameState, otherArmy)
{
...
PETRA_EXPERT.DefenseArmy.prototype.needsDefenders = function(gameState)
{
	let defenseRatio;
...
PETRA_EXPERT.DefenseArmy.prototype.recalculatePosition = function(gameState, force)
{
...
PETRA_EXPERT.DefenseArmy.prototype.recalculateStrengths = function(gameState)
{
	this.ownStrength = 0;
	this.foeStrength = 0;
...
PETRA_EXPERT.DefenseArmy.prototype.evaluateStrength = function(ent, isOwn, remove)
{
	if (!ent)
		return;
	...
	if (ent.hasClass("Structure"))
	{
		...
		entStrength = PETRA_EXPERT.getMaxStrength(ent, this.Config.debug, this.Config.DamageTypeImportance);
...
PETRA_EXPERT.DefenseArmy.prototype.checkEvents = function(gameState, events)
{
...
PETRA_EXPERT.DefenseArmy.prototype.update = function(gameState)
{
	for (let entId of this.ownEntities)
	{
		let ent = gameState.getEntityById(entId);
		...
		else if (orderData.length && orderData[0].target && orderData[0].attackType && orderData[0].attackType === "Capture")
		{
			...
			if (target && !PETRA_EXPERT.allowCapture(gameState, ent, target))
...
PETRA_EXPERT.DefenseArmy.prototype.Serialize = function()
{
	return {
		"ID": this.ID,
		"type": this.type,
...
PETRA_EXPERT.DefenseArmy.prototype.Deserialize = function(data)
{
	for (let key in data)
		this[key] = data[key];
};
