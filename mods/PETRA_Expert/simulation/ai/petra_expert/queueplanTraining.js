PETRA_EXPERT.TrainingPlan = function(gameState, type, metadata, number = 1, maxMerge = 5)
{
...
PETRA_EXPERT.TrainingPlan.prototype = Object.create(PETRA_EXPERT.QueuePlan.prototype);

PETRA_EXPERT.TrainingPlan.prototype.canStart = function(gameState)
{
...
/* PETRA_EXPERT.TrainingPlan.prototype.getBestTrainers = function(gameState)
{
...
PETRA_EXPERT.TrainingPlan.prototype.getBestTrainers = function(gameState) {
    ...
    // Find trainer with smallest queue
    let bestTrainer = null;
    let minQueue = Infinity;

    for (let trainer of allTrainers.values()) {
        if (trainer.foundationProgress() !== undefined)
            continue; // Skip trainers that are still being built

        let queueLength = trainer.trainingQueue() ? trainer.trainingQueue().length : 0;
    ...
    // If no trainer has an empty queue, return undefined
    if (!bestTrainer || minQueue > 0)
        return undefined;

    return [bestTrainer];
};

PETRA_EXPERT.TrainingPlan.prototype.start = function(gameState)
{
	if (this.metadata && this.metadata.trainer)
	{
		let metadata = {};
	...
	if (this.trainers.length > 1)
	{
		...
		const workerUnit = this.metadata && this.metadata.role && this.metadata.role === PETRA_EXPERT.Worker.ROLE_WORKER;
		let supportUnit = this.template.hasClass("Support");

		this.trainers.sort(function(a, b) {
			// Prefer training buildings with short queues
			let aa = a.trainingQueue().length === 0 ? 0 : 100;
        	let bb = b.trainingQueue().length === 0 ? 0 : 100;

        	// 
        	aa += a.trainingQueueTime();
        	bb += b.trainingQueueTime();

			
			if (a.hasClass("House") && supportUnit)
				 aa -= 30;
			//
			if (b.hasClass("House") && supportUnit)
			 	bb -= 30;
...
PETRA_EXPERT.TrainingPlan.prototype.onStart = function(gameState)
{
...
PETRA_EXPERT.TrainingPlan.prototype.addItem = function(amount = 1)
{
	this.number += amount;
};

PETRA_EXPERT.TrainingPlan.prototype.Serialize = function()
{
	return {
...
PETRA_EXPERT.TrainingPlan.prototype.Deserialize = function(gameState, data)
{
	for (let key in data)
		this[key] = data[key];
