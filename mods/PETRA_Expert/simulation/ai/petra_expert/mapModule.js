...
PETRA_EXPERT.TERRITORY_PLAYER_MASK = 0x1F;
PETRA_EXPERT.TERRITORY_BLINKING_MASK = 0x40;

PETRA_EXPERT.createObstructionMap = function(gameState, accessIndex, template)
{
	...
	{
		let tilePlayer = territoryMap.data[k] & PETRA_EXPERT.TERRITORY_PLAYER_MASK;
		let isConnected = (territoryMap.data[k] & PETRA_EXPERT.TERRITORY_BLINKING_MASK) == 0;
...
PETRA_EXPERT.createTerritoryMap = function(gameState)
{
	let map = gameState.ai.territoryMap;

	let ret = new API3.Map(gameState.sharedScript, "territory", map.data);
	ret.getOwner = function(p) { return this.point(p) & PETRA_EXPERT.TERRITORY_PLAYER_MASK; };
	ret.getOwnerIndex = function(p) { return this.map[p] & PETRA_EXPERT.TERRITORY_PLAYER_MASK; };
	ret.isBlinking = function(p) { return (this.point(p) & PETRA_EXPERT.TERRITORY_BLINKING_MASK) != 0; };
	return ret;
};
...
PETRA_EXPERT.outside_Mask = 1;
PETRA_EXPERT.border_Mask = 2;
PETRA_EXPERT.fullBorder_Mask = PETRA_EXPERT.outside_Mask | PETRA_EXPERT.border_Mask;
PETRA_EXPERT.narrowFrontier_Mask = 4;
PETRA_EXPERT.largeFrontier_Mask = 8;
PETRA_EXPERT.fullFrontier_Mask = PETRA_EXPERT.narrowFrontier_Mask | PETRA_EXPERT.largeFrontier_Mask;

PETRA_EXPERT.createBorderMap = function(gameState)
{
...
PETRA_EXPERT.debugMap = function(gameState, map)
{
	let width = map.width;
