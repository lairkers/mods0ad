...
PETRA_EXPERT.Queue.prototype.empty = function()
{
	this.plans = [];
};

PETRA_EXPERT.Queue.prototype.addPlan = function(newPlan)
{
	if (!newPlan)
		return;
...
PETRA_EXPERT.Queue.prototype.check= function(gameState)
{
...
PETRA_EXPERT.Queue.prototype.getNext = function()
{
...
PETRA_EXPERT.Queue.prototype.startNext = function(gameState)
{
	if (this.plans.length > 0)
	{
 ...
 */
PETRA_EXPERT.Queue.prototype.maxAccountWanted = function(gameState, fraction)
{
...
PETRA_EXPERT.Queue.prototype.queueCost = function()
{
	let cost = new API3.Resources();
	for (let plan of this.plans)
...
PETRA_EXPERT.Queue.prototype.length = function()
{
	return this.plans.length;
};

PETRA_EXPERT.Queue.prototype.hasQueuedUnits = function()
{
	return this.plans.length > 0;
};

PETRA_EXPERT.Queue.prototype.countQueuedUnits = function()
{
	let count = 0;
	for (let plan of this.plans)
		count += plan.number;
...
PETRA_EXPERT.Queue.prototype.hasQueuedUnitsWithClass = function(classe)
{
	return this.plans.some(plan => plan.template && plan.template.hasClass(classe));
};

PETRA_EXPERT.Queue.prototype.countQueuedUnitsWithClass = function(classe)
{
	let count = 0;
	for (let plan of this.plans)
...
PETRA_EXPERT.Queue.prototype.countQueuedUnitsWithMetadata = function(data, value)
{
	let count = 0;
	for (let plan of this.plans)
...
PETRA_EXPERT.Queue.prototype.Serialize = function()
{
	let plans = [];
	for (let plan of this.plans)
...
PETRA_EXPERT.Queue.prototype.Deserialize = function(gameState, data)
{
	...
	for (let dataPlan of data.plans)
	{
		...
		if (dataPlan.category == "unit")
			plan = new PETRA_EXPERT.TrainingPlan(gameState, dataPlan.type);
		else if (dataPlan.category == "building")
			plan = new PETRA_EXPERT.ConstructionPlan(gameState, dataPlan.type);
		else if (dataPlan.category == "technology")
			plan = new PETRA_EXPERT.ResearchPlan(gameState, dataPlan.type);
		else
		{
			API3.warn("Petra Expert deserialization error: plan unknown " + uneval(dataPlan));
			continue;
		}
