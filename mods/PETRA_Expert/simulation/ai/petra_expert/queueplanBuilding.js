...
PETRA_EXPERT.ConstructionPlan = function(gameState, type, metadata, position)
{
	if (!PETRA_EXPERT.QueuePlan.call(this, gameState, type, metadata))
		return false;
...
PETRA_EXPERT.ConstructionPlan.prototype = Object.create(PETRA_EXPERT.QueuePlan.prototype);

PETRA_EXPERT.ConstructionPlan.prototype.canStart = function(gameState)
{
...
PETRA_EXPERT.ConstructionPlan.prototype.start = function(gameState)
{
	...
	if (!pos)
	{
		gameState.ai.HQ.buildManager.setUnbuildable(gameState, this.type, 25, "room");
		Engine.ProfileStop();
		return;
	}
...
PETRA_EXPERT.ConstructionPlan.prototype.findGoodPosition = function(gameState)
{
	...
	// TEST Farmstead
	if (template.hasClass("Farmstead") && this.metadata && this.metadata.base)
	{
		// recompute the best dropsite location in case some conditions have changed
		let base = HQ.getBaseByID(this.metadata.base);
		let type = this.metadata.type ? this.metadata.type : "food";
		const newpos = base.findBestDropsiteLocation(gameState, type, template._templateName);
		if (newpos && newpos.quality > 0)
		{
			let pos = newpos.pos;
	...
	else	// No position was specified so try and find a sensible place to build
	{
		...
		if (!HQ.requireHouses || !template.hasClass("House"))
		{
			gameState.getOwnStructures().forEach(function(ent) {
				...
				let struct = PETRA_EXPERT.getBuiltEntity(gameState, ent);
				...
				else if (template.hasClass("House"))
				{
					...
					{
						placement.addInfluence(x, z, 80 / cellSize, 40);    // houses are close to other houses
						alreadyHasHouses = true;
					}
					else if (ent.hasClass("House"))
						{
							placement.addInfluence(x, z, 50 / cellSize, 40);    // houses are close to other houses
							//alreadyHasHouses = true;
						}
					...
					ent.hasClasses(["Storehouse", "House"]))
					placement.addInfluence(x, z, 40 / cellSize, 80);
				// else if (template.hasClass("Field"))
				// {
				// 	if (ent.hasClass("Field"))
				// 	{
				// 		placement.addInfluence(x, z, 80 / cellSize, 100);
    //
				// 	}
				// }
				// else if (template.genericName() == "Fruit")
				// {
    ...
    //
				...
				// {
				// 	if (ent.hasClass("Fruit"))
				// 	{
				// 		placement.addInfluence(x, z, 80 / cellSize, 255);
    //
				// 	}
				// }
				else if (template.hasClass("Farmstead") && !ent.hasClasses(["Field", "Corral", "Fruit"]) &&
					ent.hasClasses(["Gate", "!Wall"]))
					placement.addInfluence(x, z, 80 / cellSize, -25);       // move farmsteads away to make room (Wall test needed for iber)
				else if (template.hasClass("GarrisonFortress") && ent.hasClass("House"))
					placement.addInfluence(x, z, 60 / cellSize, -50);
				else if (template.hasClass("Military"))
				{
					if (ent.hasClass("Military"))
					{
						placement.addInfluence(x, z, 50 / cellSize, 100);
				...
				else if (template.hasClass("Military"))
				{
					if (ent.hasClass("Farmstead"))
					{
						placement.addInfluence(x, z, 50 / cellSize, -50);
				...
				else if (template.genericName() == "Fruit" && ent.hasClass("Farmstead"))
					placement.addInfluence(x, z, 40 / cellSize, 255);
			});
		}
		...
		// 		if (HQ.borderMap.map[j] & PETRA_EXPERT.fullBorder_Mask)
	...
	if (this.metadata && this.metadata.base !== undefined)
	{
		...
		for (let j = 0; j < placement.map.length; ++j)
		{
			...
			{
				if (favorBorder && HQ.borderMap.map[j] & PETRA_EXPERT.border_Mask)
					placement.set(j, placement.map[j] + 50);
				else if (disfavorBorder && !(HQ.borderMap.map[j] & PETRA_EXPERT.fullBorder_Mask))
	...
	if (template.hasClass("House") && !alreadyHasHouses)
	{
		...
		bestTile = placement.findBestTile(1.15*radius, obstructions);
		if (!bestTile.val)
			bestTile = undefined;
	}
	...
	let x = (bestIdx % obstructions.width + 0.25) * obstructions.cellSize;
	let z = (Math.floor(bestIdx / obstructions.width) + 0.25) * obstructions.cellSize;
 ...
 */
PETRA_EXPERT.ConstructionPlan.prototype.findDockPosition = function(gameState)
{
	let template = this.template;
	...
	for (let j = 0; j < territoryMap.length; ++j)
	{
		...
		if (oversea)
		{
			...
			{
				if (PETRA_EXPERT.getSeaAccess(gameState, dock) != navalPassMap[i])
					continue;
		...
		if (gameState.ai.HQ.borderMap.map[j] & PETRA_EXPERT.fullBorder_Mask)
			score += 20;
 ...
 */
PETRA_EXPERT.ConstructionPlan.prototype.buildOverseaDock = function(gameState, template)
{
	...
	for (let i = 0; i < gameState.ai.accessibility.regionSize.length; ++i)
	{
		...
		for (let dock of docks.values())
		{
			if (PETRA_EXPERT.getLandAccess(gameState, dock) != i)
				continue;
			keep = false;
			break;
		}
		...
		for (let cc of ccEnts.values())
		{
			let ccAccess = PETRA_EXPERT.getLandAccess(gameState, cc);
			if (ccAccess != i)
			{
...
PETRA_EXPERT.ConstructionPlan.prototype.getDockAngle = function(gameState, x, z, size)
{
 ...
 */
PETRA_EXPERT.ConstructionPlan.prototype.checkDockPlacement = function(gameState, x, z, halfDepth, halfWidth, angle)
{
 ...
 */
PETRA_EXPERT.ConstructionPlan.prototype.around = [[ 1.0, 0.0], [ 0.87, 0.50], [ 0.50, 0.87], [ 0.0, 1.0], [-0.50, 0.87], [-0.87, 0.50],
	        [-1.0, 0.0], [-0.87, -0.50], [-0.50, -0.87], [ 0.0, -1.0], [ 0.50, -0.87], [ 0.87, -0.50]];

PETRA_EXPERT.ConstructionPlan.prototype.isDockLocation = function(gameState, j, dimension, wantedLand, wantedSea)
{
	let width = gameState.ai.HQ.territoryMap.width;
	let cellSize = gameState.ai.HQ.territoryMap.cellSize;
 ...
 */
PETRA_EXPERT.ConstructionPlan.prototype.getFrontierProximity = function(gameState, j)
{
	...
	for (let a of this.around)
	{
		for (let i = 1; i < 5; ++i)
		{
			...
			if (jz < 0 || jz >= width)
				continue;
			if (borderMap.map[jx+width*jz] & PETRA_EXPERT.outside_Mask)
				continue;
 ...
 */
PETRA_EXPERT.ConstructionPlan.prototype.getResourcesAround = function(gameState, types, i, radius)
{
...
PETRA_EXPERT.ConstructionPlan.prototype.isGo = function(gameState)
{
...
PETRA_EXPERT.ConstructionPlan.prototype.onStart = function(gameState)
{
...
PETRA_EXPERT.ConstructionPlan.prototype.Serialize = function()
{
	return {
...
PETRA_EXPERT.ConstructionPlan.prototype.Deserialize = function(gameState, data)
{
	for (let key in data)
		this[key] = data[key];
