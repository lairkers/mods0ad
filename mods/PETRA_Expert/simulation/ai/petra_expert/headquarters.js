 ...
 */
PETRA_EXPERT.HQ = function(Config)
{
	this.Config = Config;
	...
	this.extraTowers = Math.round(randFloat(5, 15) * this.Config.personality.defensive);
	this.extraFortresses = 1;

	this.basesManager = new PETRA_EXPERT.BasesManager(this.Config);
	this.attackManager = new PETRA_EXPERT.AttackManager(this.Config);
	this.buildManager = new PETRA_EXPERT.BuildManager();
	this.defenseManager = new PETRA_EXPERT.DefenseManager(this.Config);
	this.tradeManager = new PETRA_EXPERT.TradeManager(this.Config);
	this.navalManager = new PETRA_EXPERT.NavalManager(this.Config);
	this.researchManager = new PETRA_EXPERT.ResearchManager(this.Config);
	this.diplomacyManager = new PETRA_EXPERT.DiplomacyManager(this.Config);
	this.garrisonManager = new PETRA_EXPERT.GarrisonManager(this.Config);
	this.victoryManager = new PETRA_EXPERT.VictoryManager(this.Config);
	this.emergencyManager = new PETRA_EXPERT.EmergencyManager(this.Config);
...
PETRA_EXPERT.HQ.prototype.init = function(gameState, queues)
{
	this.territoryMap = PETRA_EXPERT.createTerritoryMap(gameState);
	...
	this.borderMap = PETRA_EXPERT.createBorderMap(gameState);
 ...
 */
PETRA_EXPERT.HQ.prototype.postinit = function(gameState)
{
 ...
 */
PETRA_EXPERT.HQ.prototype.getSeaBetweenIndices = function(gameState, index1, index2)
{
...
PETRA_EXPERT.HQ.prototype.checkEvents = function(gameState, events)
{
	...
	for (let evt of events.OwnershipChanged)   // capture events
	{
		...
		{
			ent.setMetadata(PlayerID, "role", PETRA_EXPERT.Worker.ROLE_TRADER);
		...
		if (ent.hasClass("Ship"))
			PETRA_EXPERT.setSeaAccess(gameState, ent);
	...
	for (let evt of events.TrainingFinished)
	{
		for (let entId of evt.entities)
		{
			...
			if (plan !== undefined && plan >= 0)
			{
				...
				if (!attack || attack.state !== PETRA_EXPERT.AttackPlan.STATE_UNEXECUTED)
					ent.setMetadata(PlayerID, "plan", -1);
			}
		}
	}
	...
	if (this.Config.difficulty < PETRA_EXPERT.DIFFICULTY_EASY)
		return;
...
PETRA_EXPERT.HQ.prototype.handleNewBase = function(gameState)
{
...
PETRA_EXPERT.HQ.prototype.checkPhaseRequirements = function(gameState, queues)
{
	...
	for (let entityReq of requirements)
	{
		if (!queues.economicBuilding.hasQueuedUnits() &&
			    !queues.militaryBuilding.hasQueuedUnits())
			{
				...
				{
					plan = new PETRA_EXPERT.ConstructionPlan(gameState, "structures/{civ}/market", { "phaseUp": true });
		...
		case "Town":
			    ...
			    !queues.militaryBuilding.hasQueuedUnits())
			{
				...
				{
					plan = new PETRA_EXPERT.ConstructionPlan(gameState, "structures/{civ}/temple", { "phaseUp": true });
					queue = "economicBuilding";
					break;
				}
				...
				{
					plan = new PETRA_EXPERT.ConstructionPlan(gameState, "structures/{civ}/forge", { "phaseUp": true });
					queue = "militaryBuilding";
					break;
				}
				if (!gameState.getOwnEntitiesByClass("Embassy", true).hasEntities() &&
				    this.canBuild(gameState, "structures/{civ}/embassy"))
				{
					plan = new PETRA_EXPERT.ConstructionPlan(gameState, "structures/{civ}/embassy", { "phaseUp": true });
					queue = "militaryBuilding";
					break;
				}
			}
		...
		default:
			...
			if (!queues[queue].hasQueuedUnits())
			{
				...
				if (structure && this.canBuild(gameState, structure))
					plan = new PETRA_EXPERT.ConstructionPlan(gameState, structure, { "phaseUp": true });
			}
		}
		...
		if (plan)
		{
			if (queue == "wonder")
			{
				gameState.ai.queueManager.changePriority("majorTech", 100, { "phaseUp": true });
				plan.queueToReset = "majorTech";
			}
			else
			{
				gameState.ai.queueManager.changePriority(queue, 400, { "phaseUp": true });
...
PETRA_EXPERT.HQ.prototype.OnPhaseUp = function(gameState, phase)
{
};


// Siege
PETRA_EXPERT.HQ.prototype.trainSiege = function(gameState, queues)
{
	// Check if the queue exists
	if (!queues.siege) {
		if (this.Config.debug > 1)
			API3.warn("siege: queue siege does not exist");
		return;
	}

	// Determine limit based on current phase
	...
	// Use the correct class format
	const siegeClasses = [["Siege"]];
	const requirements = [["strength", 1]];
	let templateDef = this.findBestTrainableUnit(gameState, siegeClasses, requirements);

	if (!templateDef) {
		if (this.Config.debug > 1)
			API3.warn("siege: No suitable template found for siege");
		return;
	}
	...
	gameState.getOwnTrainingFacilities().forEach(function(ent) {
		for (let item of ent.trainingQueue()) {
			...
			let template = gameState.getTemplate(item.unitTemplate);
			if (template && template.hasClass("Siege")) {
				numberInTraining += item.count;
			}
		}
	});

	// Use safe method to count planned units
	let numberQueued = queues.siege.countQueuedUnits();

	// Total number of champions
	let totalSiege = numberOfSiege + numberInTraining + numberQueued;

	if (this.Config.debug > 1)
		API3.warn("siege: siege - existing: " + numberOfSiege +
		", training: " + numberInTraining +
		", planned: " + numberQueued +
		", limit: " + maxSiege);

	// Exit if enough champions or too many plans in queue
	if (totalSiege >= maxSiege || queues.siege.length() >= 3) {
		if (this.Config.debug > 1)
			API3.warn("siege: Siege limit reached " + totalSiege + "/" + maxSiege);
		return;
	}

	let size = pickRandom([1, 2]);

	// Check if adding a new plan would exceed the limit
	if (totalSiege + size > maxSiege)
		size = maxSiege - totalSiege;

	if (size <= 0)
		return;

	queues.siege.addPlan(new PETRA_EXPERT.TrainingPlan(gameState, templateDef,
																	   { "role": PETRA_EXPERT.Worker.ROLE_WORKER, "base": 0 }, size, size));

};

// Elephants
PETRA_EXPERT.HQ.prototype.trainElephants = function(gameState, queues)
{
	// Check if the queue exists
	if (!queues.elephants) {
		if (this.Config.debug > 1)
			API3.warn("elephants: queue elephants does not exist");
		return;
	}
	...
	// Use the correct class format
	const elephantsClasses = ["Elephant+!Support"];
	const requirements = [["strength", 1]];
	let templateDef = this.findBestTrainableUnit(gameState, elephantsClasses, requirements);

	if (!templateDef) {
		if (this.Config.debug > 1)
			API3.warn("elephants: No suitable template found for elephants");
		return;
	}

	// Count existing champions
	let numberOfElephants = 0;
	gameState.getOwnUnits().forEach(ent => {
		if (ent.hasClass("Elephant") && !ent.hasClass("Support"))
			++numberOfElephants;
	});
	...
	gameState.getOwnTrainingFacilities().forEach(function(ent) {
		for (let item of ent.trainingQueue()) {
			...
			let template = gameState.getTemplate(item.unitTemplate);
			if (template && template.hasClass("Elephant") && !template.hasClass("Support")) {
				numberInTraining += item.count;
			}
		}
	});
	...
	// Use safe method to count planned units
	let numberQueued = queues.elephants.countQueuedUnits();

	// Total number of champions
	let totalElephants = numberOfElephants + numberInTraining + numberQueued;

	if (this.Config.debug > 1)
		API3.warn("elephants: elephants - existing: " + numberOfElephants +
		", training: " + numberInTraining +
		", planned: " + numberQueued +
		", limit: " + maxElephants);

	// Exit if enough champions or too many plans in queue
	if (totalElephants >= maxElephants || queues.elephants.length() >= 3) {
		if (this.Config.debug > 1)
			API3.warn("elephants: Elephants limit reached " + totalElephants + "/" + maxElephants);
		return;
	}
	...
	// Check if adding a new plan would exceed the limit
	if (totalElephants + size > maxElephants)
		size = maxElephants - totalElephants;

	if (size <= 0)
		return;

	queues.elephants.addPlan(new PETRA_EXPERT.TrainingPlan(gameState, templateDef,
																	   { "role": PETRA_EXPERT.Worker.ROLE_WORKER, "base": 0 }, size, size));

};


PETRA_EXPERT.HQ.prototype.trainCavalry = function(gameState, queues)
{
	// Check if the queue exists
	if (!queues.cavalry) {
		if (this.Config.debug > 1)
			API3.warn("cavalry: queue cavalry does not exist");
		return;
	}
	...
	// Use the correct class format
	const cavalryClasses = [["Cavalry"]];
	const requirements = [["strength", 1]];
	let templateDef = this.findBestTrainableUnit(gameState, cavalryClasses, requirements);

	if (!templateDef) {
		if (this.Config.debug > 1)
			API3.warn("cavalry: No suitable template found for cavalry");
		return;
	}

	// Count existing champions
	let numberOfCavalry = 0;
	gameState.getOwnUnits().forEach(ent => {
	...
	gameState.getOwnTrainingFacilities().forEach(function(ent) {
		for (let item of ent.trainingQueue()) {
			...
			let template = gameState.getTemplate(item.unitTemplate);
			if (template && template.hasClass("Cavalry")) {
				numberInTraining += item.count;
			}
		}
	});
	...
	// Use safe method to count planned units
	let numberQueued = queues.cavalry.countQueuedUnits();

	// Total number of champions
	let totalCavalry = numberOfCavalry + numberInTraining + numberQueued;

	if (this.Config.debug > 1)
		API3.warn("cavalry: cavalry - existing: " + numberOfCavalry +
		", training: " + numberInTraining +
		", planned: " + numberQueued +
		", limit: " + maxCavalry);

	// Exit if enough champions or too many plans in queue
	if (totalCavalry >= maxCavalry || queues.cavalry.length() >= 3) {
		if (this.Config.debug > 1)
			API3.warn("cavalry: Cavalry limit reached " + totalCavalry + "/" + maxCavalry);
		return;
	}
	...
	// Check if adding a new plan would exceed the limit
	if (totalCavalry + size > maxCavalry)
		size = maxCavalry - totalCavalry;

	if (size <= 0)
		return;

	queues.cavalry.addPlan(new PETRA_EXPERT.TrainingPlan(gameState, templateDef,
																	   { "role": PETRA_EXPERT.Worker.ROLE_WORKER, "base": 0 }, size, size));

};

PETRA_EXPERT.HQ.prototype.trainChampionInfantryMelee = function(gameState, queues)
{
    // Check if the queue exists
    if (!queues.championInfantryMelee) {
        if (this.Config.debug > 1)
            API3.warn("trainChampionInfantryMelee: queue championInfantryMelee does not exist");
        return;
    }
    ...
    // Use the correct class format
    const championClasses = ["Champion"];
    const requirements = [["strength", 1]];
    let templateDef = this.findBestTrainableUnit(gameState, championClasses, requirements);
    
    if (!templateDef) {
        if (this.Config.debug > 1)
            API3.warn("trainChampionInfantryMelee: No suitable template found for Champion");
        return;
    }
    
    // Count existing champions
    let numberOfChampions = 0;
    gameState.getOwnUnits().forEach(ent => {
        if (ent.hasClass("Champion") && ent.hasClass("Infantry") && ent.hasClass("Melee"))
            ++numberOfChampions;
    });
    ...
    gameState.getOwnTrainingFacilities().forEach(function(ent) {
        for (let item of ent.trainingQueue()) {
            ...
            let template = gameState.getTemplate(item.unitTemplate);
            if (template && template.hasClass("Champion") && template.hasClass("Infantry") && template.hasClass("Melee")) {
                numberInTraining += item.count;
            }
        }
    });
    ...
    // Use safe method to count planned units
    let numberQueued = queues.championInfantryMelee.countQueuedUnits();
    
    // Total number of champions
    let totalChampions = numberOfChampions + numberInTraining + numberQueued;
    
    if (this.Config.debug > 1)
        API3.warn("trainChampionInfantryMelee: Champions - existing: " + numberOfChampions + 
                 ", training: " + numberInTraining + 
                 ", planned: " + numberQueued + 
                 ", limit: " + maxChampions);
    
    // Exit if enough champions or too many plans in queue
    if (totalChampions >= maxChampions || queues.championInfantryMelee.length() >= 3) {
        if (this.Config.debug > 1)
            API3.warn("trainChampionInfantryMelee: Champion limit reached " + totalChampions + "/" + maxChampions);
        return;
    }
    ...
    // Check if adding a new plan would exceed the limit
    if (totalChampions + size > maxChampions)
        size = maxChampions - totalChampions;
    
    if (size <= 0)
        return;
    
    queues.championInfantryMelee.addPlan(new PETRA_EXPERT.TrainingPlan(gameState, templateDef, 
        { "role": PETRA_EXPERT.Worker.ROLE_WORKER, "base": 0 }, size, size));
...
PETRA_EXPERT.HQ.prototype.trainHealer = function(gameState, queues)
{
	// Check if the queue exists
	if (!queues.healer) {
		if (this.Config.debug > 1)
			API3.warn("Healer: queue Healer does not exist");
		return;
	}
	...
	// Use the correct class format
	const healerClasses = ["Healer"];
	let requirementsDef = [ ["costsResource", 1, "food"] ];
	let templateDef = this.findBestTrainableUnit(gameState, healerClasses, requirementsDef);

	if (!templateDef) {
		if (this.Config.debug > 1)
			API3.warn("trainHealer: No suitable template found for Healer");
		return;
	}
	...
	gameState.getOwnTrainingFacilities().forEach(function(ent) {
		for (let item of ent.trainingQueue()) {
			...
			let template = gameState.getTemplate(item.unitTemplate);
			if (template && template.hasClass("Healer")) {
				numberInTraining += item.count;
			}
		}
	});
	...
	// Use safe method to count planned units
	let numberQueued = queues.healer.countQueuedUnits();

	// Total number of champions
	let totalHealers = numberOfHealers + numberInTraining + numberQueued;

	if (this.Config.debug > 1)
		API3.warn("trainHealer: Healers - existing: " + numberOfHealers +
		", training: " + numberInTraining +
		", planned: " + numberQueued +
		", limit: " + maxHealers);

	// Exit if enough champions or too many plans in queue
	if (totalHealers >= maxHealers || queues.healer.length() >= 3) {
		if (this.Config.debug > 1)
			API3.warn("trainHealer: Healers limit reached " + totalHealers + "/" + maxHealers);
		return;
	}
	...
	// Check if adding a new plan would exceed the limit
	if (totalHealers + size > maxHealers)
		size = maxHealers - totalHealers;

	if (size <= 0)
		return;

	queues.healer.addPlan(new PETRA_EXPERT.TrainingPlan(gameState, templateDef,
														 { "role": PETRA_EXPERT.Worker.ROLE_CRITICAL_ENT_HEALER, "base": 0}, size, size));

};



PETRA_EXPERT.HQ.prototype.trainSupport = function(gameState, queues)
{
	// Check if the queue exists
	if (!queues.support) {
		if (this.Config.debug > 1)
			API3.warn("support: queue support does not exist");
		return;
	}
	...
	// Use the correct class format
	const supportClasses = ["Support+Worker"];
	let requirementsDef = [ ["costsResource", 1, "food"] ];
	let templateDef = this.findBestTrainableUnit(gameState, supportClasses, requirementsDef);

	if (!templateDef) {
		if (this.Config.debug > 1)
			API3.warn("trainSupport: No suitable template found for Support");
		return;
	}
	...
	gameState.getOwnUnits().forEach(ent => {
		if (ent.hasClass("Support") && ent.hasClass("Worker"))
			++numberOfSupports;
	});
	...
	gameState.getOwnTrainingFacilities().forEach(function(ent) {
		for (let item of ent.trainingQueue()) {
			...
			let template = gameState.getTemplate(item.unitTemplate);
			if (template && template.hasClass("Support") && template.hasClass("Worker")) {
				numberInTraining += item.count;
			}
		}
	});
	...
	// Use safe method to count planned units
	let numberQueued = queues.support.countQueuedUnits();

	// Total number of champions
	let totalSupports = numberOfSupports + numberInTraining + numberQueued;

	if (this.Config.debug > 1)
		API3.warn("trainSupport: Supports - existing: " + numberOfSupports +
		", training: " + numberInTraining +
		", planned: " + numberQueued +
		", limit: " + maxSupports);

	// Exit if enough champions or too many plans in queue
	if (totalSupports >= maxSupports || queues.support.length() >= 3) {
		if (this.Config.debug > 1)
			API3.warn("trainSupport: Supports limit reached " + totalSupports + "/" + maxSupports);
		return;
	}

	let size = pickRandom([1, 2, 3]);

	// Check if adding a new plan would exceed the limit
	if (totalSupports + size > maxSupports)
		size = maxSupports - totalSupports;

	if (size <= 0)
		return;

	queues.support.addPlan(new PETRA_EXPERT.TrainingPlan(gameState, templateDef,
																	   { "role": PETRA_EXPERT.Worker.ROLE_WORKER, "base": 0, "support": true }, size, size));

};



PETRA_EXPERT.HQ.prototype.trainCitizenSoldiersMelee = function(gameState, queues)
{
	// Check if the queue exists
	if (!queues.citizenSoldierMelee) {
		if (this.Config.debug > 1)
			API3.warn("citizenSoldierMelee: queue citizenSoldierMelee does not exist");
		return;
	}

	// Determine limit based on current phase
	let currentPhase = gameState.currentPhase();
	let maxCitizenSoldiersMelee = 40;

	if (currentPhase == 1)
		maxCitizenSoldiersMelee = 15;
	else if (currentPhase == 2)
		maxCitizenSoldiersMelee = 25;
	else
		maxCitizenSoldiersMelee = 35;

	// Use the correct class format
	//const citizenSoldierClasses = ["Infantry"];
	//const classes = [["CitizenSoldier", "Infantry", "!Mercenary"]];
	//
	// 		// We want at least 33% ranged and 33% melee.
	// 		classes[0].push(pickRandom(["Ranged", "Melee", "Infantry"]));
	let requirementsDef = [ ["costsResource", 1, "food"] ];
	const citizenSoldierClasses = [["CitizenSoldier", "Infantry", "!Mercenary"]];



// 		// We want at least 33% ranged and 33% melee.
	citizenSoldierClasses[0].push(pickRandom(["Ranged", "Melee", "Infantry"]));
	//citizenSoldierClasses[0].push(pickRandom(["Ranged", "Melee", "Infantry"]));

	let templateDef = this.findBestTrainableUnit(gameState, citizenSoldierClasses, requirementsDef);

	if (!templateDef) {
		if (this.Config.debug > 1)
			API3.warn("citizenSoldierMelee: No suitable template found for citizenSoldierMelee");
		return;
	}

	// Count existing champions
	let numberOfCitizenSoldiersMelee = 0;
	// gameState.getOwnUnits().forEach(ent => {
	// 	if (ent.hasClass("Infantry") && ent.hasClass("Worker") && ent.getMetadata(PlayerID, "role") === PETRA_EXPERT.Worker.ROLE_WORKER && ent.getMetadata(PlayerID, "plan") === undefined)
	// 		++numberOfCitizenSoldiersMelee;
	// });
	gameState.getOwnUnits().forEach(ent => {
		if (ent.hasClass("Worker") && !ent.hasClass("Support") && ent.getMetadata(PlayerID, "plan") === undefined)
			++numberOfCitizenSoldiersMelee;
	});
	...
	gameState.getOwnTrainingFacilities().forEach(function(ent) {
		for (let item of ent.trainingQueue()) {
			...
			let template = gameState.getTemplate(item.unitTemplate);
			if (template && template.hasClass("Worker") && template.hasClass("Melee")) {
				numberInTraining += item.count;
			}
		}
	});
	...
	// Use safe method to count planned units
	let numberQueued = queues.citizenSoldierMelee.countQueuedUnits();

	// Total number of champions
	let totalCitizenSoldiersMelee = numberOfCitizenSoldiersMelee + numberInTraining + numberQueued;

	if (this.Config.debug > 1)
		API3.warn("trainCitizenSoldiersMelee: CitizenSoldiersMelee - existing: " + numberOfCitizenSoldiersMelee +
		", training: " + numberInTraining +
		", planned: " + numberQueued +
		", limit: " + maxCitizenSoldiersMelee);

	// Exit if enough champions or too many plans in queue
	if (totalCitizenSoldiersMelee >= maxCitizenSoldiersMelee || queues.citizenSoldierMelee.length() >= 3) {
		if (this.Config.debug > 1)
			API3.warn("trainCitizenSoldiersMelee: citizenSoldiersMelee limit reached " + totalCitizenSoldiersMelee + "/" + maxCitizenSoldiersMelee);
		return;
	}
	...
	// Check if adding a new plan would exceed the limit
	if (totalCitizenSoldiersMelee + size > maxCitizenSoldiersMelee)
		size = maxCitizenSoldiersMelee - totalCitizenSoldiersMelee;

	if (size <= 0)
		return;

	queues.citizenSoldierMelee.addPlan(new PETRA_EXPERT.TrainingPlan(gameState, templateDef,
														 { "role": PETRA_EXPERT.Worker.ROLE_WORKER, "base": 0}, size, size));
...
PETRA_EXPERT.HQ.prototype.trainHero = function(gameState, queues)
{
	// Check if the queue exists
	if (!queues.hero) {
		if (this.Config.debug > 1)
			API3.warn("hero: queue hero does not exist");
		return;
	}
	...
	let requirementsDef = [ ["costsResource", 1, "food"] ];
	let templateDef = this.findBestTrainableUnit(gameState, heroClasses, requirementsDef);

	if (!templateDef) {
		if (this.Config.debug > 1)
			API3.warn("trainHero: No suitable template found for Hero");
		return;
	}
	...
	gameState.getOwnTrainingFacilities().forEach(function(ent) {
		for (let item of ent.trainingQueue()) {
			...
			let template = gameState.getTemplate(item.unitTemplate);
			if (template && template.hasClass("Hero")) {
				numberInTraining += item.count;
			}
		}
	});
	...
	// Total number of champions
	let totalHero = numberOfHero + numberInTraining + numberQueued;

	if (this.Config.debug > 1)
		API3.warn("trainHero: Hero - existing: " + numberOfHero +
		", training: " + numberInTraining +
		", planned: " + numberQueued +
		", limit: " + maxHero);

	// Exit if enough champions or too many plans in queue
	if (totalHero >= maxHero || queues.hero.length() >= 1) {
		if (this.Config.debug > 1)
			API3.warn("trainHero: Hero limit reached " + totalHero + "/" + maxHero);
		return;
	}

	queues.hero.addPlan(new PETRA_EXPERT.TrainingPlan(gameState, templateDef,
														 { "role": PETRA_EXPERT.Worker.ROLE_WORKER, "base": 0}, 1, 1));

};


// PETRA_EXPERT.HQ.prototype.trainCitizenSoldiersRanged = function(gameState, queues)
// {
// 	// Check if the queue exists
// 	if (!queues.citizenSoldierRanged) {
// 		if (this.Config.debug > 1)
// 			API3.warn("citizenSoldierRanged: queue citizenSoldierRanged does not exist");
// 		return;
// 	}

// 	// Determine limit based on current phase
// 	let currentPhase = gameState.currentPhase();
// 	let maxCitizenSoldiersRanged;

// 	if (currentPhase == 1)
// 		maxCitizenSoldiersRanged = 35;
// 	else if (currentPhase == 2)
// 		maxCitizenSoldiersRanged = 45;
// 	else
// 		maxCitizenSoldiersRanged = 45;
...
// 	//const citizenSoldierClasses = ["Infantry"];
// 	let requirementsDef = [ ["costsResource", 1, "food"] ];
// 	const citizenSoldierClasses = [["Infantry+Ranged"]];

// 	// We want at least 33% ranged and 33% melee.
// 	//citizenSoldierClasses[0].push(["Ranged"]);
// 	let templateDef = this.findBestTrainableUnit(gameState, citizenSoldierClasses, requirementsDef);

// 	if (!templateDef) {
// 		if (this.Config.debug > 1)
// 			API3.warn("citizenSoldierRanged: No suitable template found for citizenSoldierRanged");
// 		return;
// 	}

// 	// Count existing champions
// 	let numberOfCitizenSoldiersRanged = 0;
// 	gameState.getOwnUnits().forEach(ent => {
// 		if (ent.hasClass("Infantry") && ent.hasClass("Ranged") && ent.getMetadata(PlayerID, "role") === PETRA_EXPERT.Worker.ROLE_WORKER && ent.getMetadata(PlayerID, "plan") === undefined)
// 			++numberOfCitizenSoldiersRanged;
// 	});
...
// 			let template = gameState.getTemplate(item.unitTemplate);
// 			if (template && template.hasClass("Infantry") && template.hasClass("Ranged")) {
// 				numberInTraining += item.count;
// 			}
// 		}
// 	});
...
// 	// Use safe method to count planned units
// 	let numberQueued = queues.citizenSoldierRanged.countQueuedUnits();

// 	// Total number of champions
// 	let totalCitizenSoldiersRanged = numberOfCitizenSoldiersRanged + numberInTraining + numberQueued;

// 	if (this.Config.debug > 1)
// 		API3.warn("trainCitizenSoldiersRanged: CitizenSoldiersRanged - existing: " + numberOfCitizenSoldiersRanged +
// 		", training: " + numberInTraining +
// 		", planned: " + numberQueued +
// 		", limit: " + maxCitizenSoldiersRanged);

// 	// Exit if enough champions or too many plans in queue
// 	if (totalCitizenSoldiersRanged >= maxCitizenSoldiersRanged || queues.citizenSoldierRanged.length() >= 3) {
// 		if (this.Config.debug > 1)
// 			API3.warn("trainCitizenSoldiersRanged: citizenSoldiersRanged limit reached " + totalCitizenSoldiersRanged + "/" + maxCitizenSoldiersRanged);
// 		return;
// 	}
...
// 	// Check if adding a new plan would exceed the limit
// 	if (totalCitizenSoldiersRanged + size > maxCitizenSoldiersRanged)
// 		size = maxCitizenSoldiersRanged - totalCitizenSoldiersRanged;

// 	if (size <= 0)
// 		return;

// 	queues.citizenSoldierRanged.addPlan(new PETRA_EXPERT.TrainingPlan(gameState, templateDef,
// 														 { "role": PETRA_EXPERT.Worker.ROLE_WORKER, "base": 0}, size, size));
...
// PETRA_EXPERT.HQ.prototype.trainMoreWorkers = function(gameState, queues)
// {
// 	// default template
// 	let requirementsDef = [ ["costsResource", 1, "food"] ];
...
// 	gameState.getOwnUnits().forEach(ent => {
// 		if (ent.getMetadata(PlayerID, "role") === PETRA_EXPERT.Worker.ROLE_WORKER && ent.getMetadata(PlayerID, "plan") === undefined)
// 		{
// 			++numberOfWorkers;
...
// 		{
// 			numberInTraining += item.count;
// 			if (item.metadata && item.metadata.role && item.metadata.role === PETRA_EXPERT.Worker.ROLE_WORKER &&
...
// 			villager: this.Config.priorities.villager,
// 			citizenSoldier: this.Config.priorities.citizenSoldier,
// 			trader: this.Config.priorities.trader,
// 			healer: this.Config.priorities.healer,
// 			ships: this.Config.priorities.ships,
// 			house: this.Config.priorities.house,
// 			dropsites: this.Config.priorities.dropsites,
// 			field: this.Config.priorities.field,
// 			dock: this.Config.priorities.dock,
// 			corral: this.Config.priorities.corral,
// 			economicBuilding: this.Config.priorities.economicBuilding,
// 			militaryBuilding: this.Config.priorities.militaryBuilding,
// 			defenseBuilding: this.Config.priorities.defenseBuilding,
// 			civilCentre: this.Config.priorities.civilCentre,
// 			majorTech: this.Config.priorities.majorTech,
// 			minorTech: this.Config.priorities.minorTech,
// 			wonder: this.Config.priorities.wonder,
// 			emergency: this.Config.priorities.emergency
// 		};
//     }
//
//
...
// 	//let size = Math.ceil(randFloat(0, 7)); //= numberOfWorkers < 12 ? 1 : 2; //randBatchSize Math.min(5, Math.ceil(numberOfWorkers / 10));
//
//
//
//
...
//
// 	let villagerRandSize = numberOfWorkers < 7 ? 1 : pickRandom([2, 3, 4]);
//
// 	//let growingSize = numberOfWorkers < 12 ? pickRandom([2, 3, 4]) : Math.min(10, Math.ceil(numberOfWorkers / 10));
//
// 	//let size = pickRandom([moreRandomSize, growingSize, 1]);
// 	//let size = numberOfWorkers < 12 ? pickRandom([2, 3, 4]) : Math.min(10, Math.ceil(numberOfWorkers / 10));
//
// 	let size = pickRandom([1, 2, 3]);
//
// 	//let villagerSize = available.food > 50 ? Math.min(10, Math.floor(available.food / 50)) : villagerRandSize;
//
// 	let villagerSize = villagerRandSize;
//
//
...
//
// 		//queues.villager.plans[0].number = size;
// 		//if (queues.villager.plans[1])
// 			//queues.villager.plans[1].number = size;
//
// 		queues.villager.plans[0].number = villagerSize;
// 		if (queues.villager.plans[1])
// 			queues.villager.plans[1].number = villagerSize;
// 	}
...
//
// 		queues.citizenSoldier.plans[0].number = size;
// 		if (queues.citizenSoldier.plans[1])
// 			queues.citizenSoldier.plans[1].number = size;
// 	}
//
...
//
//
// 	/* if (this.emergencyManager.hasEmergency)
// 	{
// 		gameState.ai.queueManager.changePriority("citizenSoldier", this.Config.priorities.citizenSoldier * 10);
//
// 	} */
//
// 	//
// 	 if (numberOfSupports + numberOfQueuedSupports < 30 && !this.emergencyManager.hasEmergency) {
// 		//
// 		if (!this.prioritiesModified || queues.villager.paused) {
// 			gameState.ai.queueManager.changePriority("villager", this.Config.priorities.villager * 10);
// 			this.prioritiesModified = true;
// 			queues.villager.paused = false;
// 		}
// 	}
// 	else if (numberOfSupports + numberOfQueuedSupports >= 50) {
// 		//
// 		if (this.prioritiesModified) {
// 			gameState.ai.queueManager.pauseQueue("villager");
//
// 			this.prioritiesModified = false;
...
// 		supportRatio = Math.min(this.supportRatio, 1);
// 	if (this.attackManager.rushNumber < this.attackManager.maxRushes || this.attackManager.upcomingAttacks[PETRA_EXPERT.AttackPlan.TYPE_RUSH].length)
// 		alpha = .95;
...
// 		let requirements;
// 		if (numberTotal < 50)
// 			requirements = [ ["speed", 0.5], ["costsResource", 0.5, "food"], ["costsResource", 0.3, "metal"] ];
// 		else
// 		requirements = [ ["strength", .33], ["costsResource", 0.33, "food"], ["costsResource", 0.33, "metal"] ];
//
// 		const classes = [["CitizenSoldier", "Infantry", "!Mercenary"]];
//
// 		// We want at least 33% ranged and 33% melee.
...
//
//
// 	/* let trainers = gameState.findTrainers(templateDef);
// 	let useHouseSize = false;
//
//
// 	for (let trainer of trainers) {
// 		if (trainer.hasClass("House")) {
// 			useHouseSize = true;
// 			break;
// 		}
// 	}
//
//
// 	// For houses use size 1, for others - standard logic
// 	let finalVillagerSize = useHouseSize ? 1 : villagerSize; */
//
...
// 		//queues.villager.addPlan(new PETRA_EXPERT.TrainingPlan(gameState, templateDef, { "role": PETRA_EXPERT.Worker.ROLE_WORKER, "base": 0, "support": true }, villagerSize, villagerSize));
// 	//if (!template && templateDef)
// 		//queues.villager.addPlan(new PETRA_EXPERT.TrainingPlan(gameState, templateDef, { "role": PETRA_EXPERT.Worker.ROLE_WORKER, "base": 0, "support": true }, villagerSize, villagerSize));
// 	//else
// 	if (template)
// 		queues.citizenSoldier.addPlan(new PETRA_EXPERT.TrainingPlan(gameState, template, { "role": PETRA_EXPERT.Worker.ROLE_WORKER, "base": 0 }, size, size));
//
// };
  

/** picks the best template based on parameters and classes */
PETRA_EXPERT.HQ.prototype.findBestTrainableUnit = function(gameState, classes, requirements)
{
	let units;
	...
	units.sort((a, b) => {
		...
		for (let param of parameters)
		{
			if (param[0] == "strength")
			{
				aValue += PETRA_EXPERT.getMaxStrength(a[1], gameState.ai.Config.debug, gameState.ai.Config.DamageTypeImportance) * param[1];
				bValue += PETRA_EXPERT.getMaxStrength(b[1], gameState.ai.Config.debug, gameState.ai.Config.DamageTypeImportance) * param[1];
			}
			else if (param[0] == "siegeStrength")
			{
				aValue += PETRA_EXPERT.getMaxStrength(a[1], gameState.ai.Config.debug, gameState.ai.Config.DamageTypeImportance, "Structure") * param[1];
				bValue += PETRA_EXPERT.getMaxStrength(b[1], gameState.ai.Config.debug, gameState.ai.Config.DamageTypeImportance, "Structure") * param[1];
			}
 ...
 */
PETRA_EXPERT.HQ.prototype.bulkPickWorkers = function(gameState, baseRef, number)
{
	return this.basesManager.bulkPickWorkers(gameState, baseRef, number);
};

PETRA_EXPERT.HQ.prototype.getTotalResourceLevel = function(gameState, resources, proximity)
{
 ...
 */
PETRA_EXPERT.HQ.prototype.GetCurrentGatherRates = function(gameState)
{
 ...
 */
PETRA_EXPERT.HQ.prototype.GetWantedGatherRates = function(gameState)
{
 ...
 */
PETRA_EXPERT.HQ.prototype.pickMostNeededResources = function(gameState, allowedResources = [])
{
 ...
 */
PETRA_EXPERT.HQ.prototype.findEconomicCCLocation = function(gameState, template, resource, proximity, fromStrategic)
{
	...
	// obstruction map
	let obstructions = PETRA_EXPERT.createObstructionMap(gameState, 0, template);
	let halfSize = 0;
	...
	for (let j = 0; j < this.territoryMap.length; ++j)
	{
		...
		else
		{
			...
			for (let cc of ccList)
			{
				...
				if (dist < minDist)
					minDist = dist;
				accessible = accessible || index == PETRA_EXPERT.getLandAccess(gameState, cc.ent);
			}
			if (norm == 0)
				continue;
			...
			oversea = !accessible && dpList.some(dp => PETRA_EXPERT.getLandAccess(gameState, dp.ent) == index);
			if (!oversea)
			{
		...
		if (this.borderMap.map[j] & PETRA_EXPERT.fullBorder_Mask)	// disfavor the borders of the map
			norm *= 0.5;

		let val = 4 * gameState.sharedScript.ccResourceMaps[resource].map[j];//2
 ...
 */
PETRA_EXPERT.HQ.prototype.findStrategicCCLocation = function(gameState, template)
{
	...
	for (let j = 0; j < this.territoryMap.length; ++j)
	{
		...
		if (this.borderMap.map[j] & PETRA_EXPERT.fullBorder_Mask)
			currentVal += 10000;
 ...
 */
PETRA_EXPERT.HQ.prototype.findMarketLocation = function(gameState, template)
{
	...
	for (let j = 0; j < this.territoryMap.length; ++j)
	{
		...
		if (this.borderMap.map[j] & PETRA_EXPERT.narrowFrontier_Mask)
			continue;
		...
		for (let market of markets)
		{
			...
			{
				if (PETRA_EXPERT.getSeaAccess(gameState, market) != gameState.ai.accessibility.getAccessValue(pos, true))
					continue;
				gainMultiplier = traderTemplatesGains.navalGainMultiplier;
			}
			else if (PETRA_EXPERT.getLandAccess(gameState, market) == index &&
				!PETRA_EXPERT.isLineInsideEnemyTerritory(gameState, market.position(), pos))
 ...
 */
PETRA_EXPERT.HQ.prototype.findDefensiveLocation = function(gameState, template)
{
	...
	for (let j = 0; j < this.territoryMap.length; ++j)
	{
		if (!wonderMode)
		{
			...
			if (!(this.borderMap.map[j] & PETRA_EXPERT.fullFrontier_Mask))
				continue;
			if (this.borderMap.map[j] & PETRA_EXPERT.largeFrontier_Mask && isTower)
				continue;
		}
...
PETRA_EXPERT.HQ.prototype.buildTemple = function(gameState, queues)
{
	...
	gameState.ai.queueManager.changePriority("economicBuilding", 10 * this.Config.priorities.economicBuilding);
	queues.economicBuilding.addPlan(new PETRA_EXPERT.ConstructionPlan(gameState, templateName));
};

PETRA_EXPERT.HQ.prototype.buildMarket = function(gameState, queues)
{
	if (gameState.getOwnEntitiesByClass("Market", true).hasEntities() ||
		!this.canBuild(gameState, "structures/{civ}/market"))
		return;
	...
	gameState.ai.queueManager.changePriority("economicBuilding", 10 * this.Config.priorities.economicBuilding);
	let plan = new PETRA_EXPERT.ConstructionPlan(gameState, "structures/{civ}/market");
	plan.queueToReset = "economicBuilding";
	queues.economicBuilding.addPlan(plan);
};

/** Build a farmstead */
PETRA_EXPERT.HQ.prototype.buildFarmstead = function(gameState, queues)
{
	let numFarmsteads = gameState.getOwnEntitiesByClass("Farmstead", true).length;
	let currentPhaseIndex = gameState.currentPhase();
	if (numFarmsteads >= 2)
		return;
	...
	if (numFarmsteads < 3)
		...
		gameState.ai.queueManager.changePriority("economicBuilding", 5 * this.Config.priorities.economicBuilding);
		queues.economicBuilding.addPlan(new PETRA_EXPERT.ConstructionPlan(gameState, "structures/{civ}/farmstead"));
 ...
 */
PETRA_EXPERT.HQ.prototype.buildWonder = function(gameState, queues, force = false)
{
	...
	queues.wonder.addPlan(new PETRA_EXPERT.ConstructionPlan(gameState, "structures/{civ}/wonder"));
};


PETRA_EXPERT.HQ.prototype.manageCorral = function(gameState, queues)
{
    ...
    // 
    let maxCorrals = Math.min(
        Math.floor(this.getAccountedPopulation(gameState) / 50),
        this.currentPhase + 3
    );
    maxCorrals = Math.min(maxCorrals, 3);

    let nCorral = gameState.getOwnEntitiesByClass("Corral", true).length;
    let corralTemplate = this.canBuild(gameState, "structures/{civ}/corral") ? "structures/{civ}/corral" : undefined;

    // 
    if (nCorral < maxCorrals &&
        this.getAccountedPopulation(gameState) >= Math.floor(45 * (nCorral + 1) * (0.8 + Math.random() * 0.4)) &&
        corralTemplate)
    {
		if (!gameState.getOwnEntitiesByClass("Storehouse", true).hasEntities())
			return;
		...
		if (!gameState.getOwnEntitiesByClass("Farmstead", true).hasEntities())
			return;
		//gameState.ai.queueManager.changePriority("corral", 5 * this.Config.priorities.corral);
        queues.corral.addPlan(new PETRA_EXPERT.ConstructionPlan(gameState, "structures/{civ}/corral"));
        return;
    }

    let corrals = gameState.getOwnEntitiesByClass("Corral", true).filter(API3.Filters.isBuilt());
    if (!corrals.hasEntities())
        return;

    // TODO: FIX Batch Size, Now not work 
    let available = gameState.ai.queueManager.getAvailableResources(gameState);
    let batchSize = 1;

    // 
    if (available.food <= 100)
        batchSize = 1;
    else if (available.food > 100 && available.food <= 200)
        batchSize = Math.ceil(randFloat(0, 2));
    else if (available.food > 200 && available.food <= 300)
        batchSize = Math.ceil(randFloat(0, 3));
    else if (available.food > 300 && available.food <= 500)
        batchSize = Math.ceil(randFloat(0, 4));
    else
        batchSize = Math.ceil(randFloat(0, 5));

    let bestCorral = null;
    let minQueue = Infinity;

    for (let corral of corrals.values()) {
        if (corral.foundationProgress() !== undefined)
            continue;

        let queueLength = corral.trainingQueue() ? corral.trainingQueue().length : 0;
    ...
    if (!bestCorral || minQueue > 0)  // max 1 plan
        return;

    let civ = gameState.getPlayerCiv();
    let trainables = bestCorral.trainableEntities(civ);
    let availableTrainables = [];
    ...
    for (let trainable of trainables)
    {
        ...
        for (let item of bestCorral.trainingQueue())
            count += item.count;

        if (count > Math.ceil(nCorral * 1.5))
            continue;

        availableTrainables.push(trainable);
    }

    if (availableTrainables.length > 0)
    {
        let randomTrainable = availableTrainables[Math.floor(Math.random() * availableTrainables.length)];
        // 
        queues.corral.addPlan(new PETRA_EXPERT.TrainingPlan(gameState, randomTrainable, {
            "trainer": bestCorral.id(),
            "number": batchSize,  //
            "batchSize": batchSize  // 
        }));
    }
 ...
 */
PETRA_EXPERT.HQ.prototype.buildMoreHouses = function(gameState, queues)
{
	...
	{
		let plan = new PETRA_EXPERT.ConstructionPlan(gameState, houseTemplateString);
	...
	if (freeSlots < 10)
	{
		if (this.buildManager.isUnbuildable(gameState, house))
		{
			...
			priority = 10 * this.Config.priorities.house;
	}
	else
...
PETRA_EXPERT.HQ.prototype.checkBaseExpansion = function(gameState, queues)
{
	...
	if (numUnits > activeBases * (80 + numvar + (15 + numvar)*(activeBases-1)) || this.saveResources && numUnits > 50)
	{
		if (this.Config.debug > 2)
...
PETRA_EXPERT.HQ.prototype.buildNewBase = function(gameState, queues, resource)
{
	...
	queues.civilCentre.addPlan(new PETRA_EXPERT.ConstructionPlan(gameState, template, { "base": -1, "resource": resource }));
	return true;
};

/** Deals with building fortresses and towers along our border with enemies. */
PETRA_EXPERT.HQ.prototype.buildDefenses = function(gameState, queues)
{
	...
	if (!this.saveResources && (this.currentPhase > 2 || gameState.isResearching(gameState.getPhaseName(3))))
	{
		...
		if (this.canBuild(gameState, "structures/{civ}/fortress"))
		{
				...
				gameState.getOwnFoundationsByClass("Fortress").length < 2)
			{
				...
				let plan = new PETRA_EXPERT.ConstructionPlan(gameState, "structures/{civ}/fortress");
	...
	// 		queues.defenseBuilding.addPlan(new PETRA_EXPERT.ConstructionPlan(gameState, "structures/{civ}/sentry_tower"));
	// 	}
	// 	return;
	// }
		...
		gameState.getOwnFoundationsByClass("Tower").length < 3)
	{
		...
		let plan = new PETRA_EXPERT.ConstructionPlan(gameState, "structures/{civ}/defense_tower");
		plan.queueToReset = "defenseBuilding";
		queues.defenseBuilding.addPlan(plan);
	}
};

PETRA_EXPERT.HQ.prototype.buildForge = function(gameState, queues)
{
	...
	if (this.canBuild(gameState, "structures/{civ}/forge"))
		queues.militaryBuilding.addPlan(new PETRA_EXPERT.ConstructionPlan(gameState, "structures/{civ}/forge"));
};
 ...
 */
PETRA_EXPERT.HQ.prototype.constructTrainingBuildings = function(gameState, queues)
{
	...
	if (this.getAccountedPopulation(gameState) > this.Config.Military.popForBarracks1 ||
	    this.phasing == 2 && gameState.getOwnStructures().filter(API3.Filters.byClass("Village")).length < 5)
		{
			...
			if (numBarracks == 0)
			{
				let template = barracksTemplate;
				if (template)
				{
					gameState.ai.queueManager.changePriority("militaryBuilding", 5 * this.Config.priorities.militaryBuilding);
					let plan = new PETRA_EXPERT.ConstructionPlan(gameState, template, { "militaryBase": true });
			...
			// Stable
			if (numStables == 0 && numBarracks >= 1 && stableTemplate && this.getAccountedPopulation(gameState) > this.Config.Military.popForStable1)
			{
				gameState.ai.queueManager.changePriority("militaryBuilding", 3 * this.Config.priorities.militaryBuilding);
				queues.militaryBuilding.addPlan(new PETRA_EXPERT.ConstructionPlan(gameState, stableTemplate, { "militaryBase": true }));
				return;
			}
			...
			if (numBarracks == 1 && this.getAccountedPopulation(gameState) >= this.Config.Military.popForBarracks2)
			{
				...
				{
					gameState.ai.queueManager.changePriority("militaryBuilding", 5 * this.Config.priorities.militaryBuilding);
					queues.militaryBuilding.addPlan(new PETRA_EXPERT.ConstructionPlan(gameState, template, { "militaryBase": true }));
					return;
				}
			}
			...
			// Range2
			if (numRanges == 1 && this.getAccountedPopulation(gameState) > this.Config.Military.popForBarracks2)
			{
				let template = rangeTemplate;
				if (template)
				{
			...
			// Stable2
			if (numStables == 1 && numBarracks >= 3 && stableTemplate && this.getAccountedPopulation(gameState) > this.Config.Military.popForStable2 && currentPhase >2)
			{
				queues.militaryBuilding.addPlan(new PETRA_EXPERT.ConstructionPlan(gameState, stableTemplate, { "militaryBase": true }));
				return;
			}
	
			// Third barracks/range and stable.
	
			// Barracks3
			if (numBarracks == 2 && this.getAccountedPopulation(gameState) > this.Config.Military.popForBarracks3)
			{
				let template = barracksTemplate;
				if (template)
				{
			...
			// Range3
			if (numRanges == 2 && this.getAccountedPopulation(gameState) > this.Config.Military.popForBarracks3)
			{
				let template = rangeTemplate;
				if (template)
				{
			...
			// Stable3
			if (numStables == 2 && stableTemplate && this.getAccountedPopulation(gameState) > this.Config.Military.popForStable3)
			{
				queues.militaryBuilding.addPlan(new PETRA_EXPERT.ConstructionPlan(gameState, stableTemplate, { "militaryBase": true }));
				return;
			}
			...
			// 4 barracks/range and stable.
	
			// Barracks4
			if (numBarracks == 3 && this.getAccountedPopulation(gameState) > this.Config.Military.popForBarracks4 && currentPhase > 2)
			{
				let template = barracksTemplate;
				if (template)
				{
			...
			// Range4
			if (numRanges == 3 && this.getAccountedPopulation(gameState) > this.Config.Military.popForBarracks4)
			{
				let template = rangeTemplate;
				if (template)
				{
			...
			// Stable4
			if (numStables == 3 && stableTemplate && this.getAccountedPopulation(gameState) > this.Config.Military.popForStable4)
			{
				queues.militaryBuilding.addPlan(new PETRA_EXPERT.ConstructionPlan(gameState, stableTemplate, { "militaryBase": true }));
				return;
			}
			...
			// 5 barracks/range and stable.
	
			// Barracks5
			if (numBarracks == 4 && this.getAccountedPopulation(gameState) > this.Config.Military.popForBarracks5 && currentPhase > 2)
			{
				let template = barracksTemplate;
				if (template)
				{
			...
			// Range5
			if (numRanges == 4 && this.getAccountedPopulation(gameState) > this.Config.Military.popForBarracks5)
			{
				let template = rangeTemplate;
				if (template)
				{
			...
			// Stable5
			if (numStables == 4 && stableTemplate && this.getAccountedPopulation(gameState) > this.Config.Military.popForStable5)
			{
				queues.militaryBuilding.addPlan(new PETRA_EXPERT.ConstructionPlan(gameState, stableTemplate, { "militaryBase": true }));
				return;
			}
	...
	// Build Special Building if Available Before 3 Phase
	if (this.getAccountedPopulation(gameState) < 30 || !this.bAdvanced.length)
		return;
	...
	if (!nAdvanced || nAdvanced < this.bAdvanced.length && this.getAccountedPopulation(gameState) > 30)
	{
		for (let advanced of this.bAdvanced)
		{
			...
			if (template.hasDefensiveFire() || template.trainableEntities(civ))
				queues.militaryBuilding.addPlan(new PETRA_EXPERT.ConstructionPlan(gameState, advanced, { "militaryBase": true }));
			else	// not a military building, but still use this queue
				queues.militaryBuilding.addPlan(new PETRA_EXPERT.ConstructionPlan(gameState, advanced));
			return;
		}
	}
	...
	if (this.canBuild(gameState, "structures/{civ}/elephant_stable") && !gameState.getOwnEntitiesByClass("ElephantStable", true).hasEntities())
		{
			gameState.ai.queueManager.changePriority("militaryBuilding", 15 * this.Config.priorities.militaryBuilding);
			queues.militaryBuilding.addPlan(new PETRA_EXPERT.ConstructionPlan(gameState, "structures/{civ}/elephant_stable", { "militaryBase": true }));
			return;
		}
	...
	{
		gameState.ai.queueManager.changePriority("militaryBuilding", 15 * this.Config.priorities.militaryBuilding);
		queues.militaryBuilding.addPlan(new PETRA_EXPERT.ConstructionPlan(gameState, "structures/{civ}/arsenal", { "militaryBase": true }));
		return;
	}
 ...
 */
PETRA_EXPERT.HQ.prototype.findBestBaseForMilitary = function(gameState)
{
	...
	for (let cce of ccEnts)
	{
		...
		if (enemyFound && !gameState.isPlayerEnemy(cce.owner()))
			continue;
		let access = PETRA_EXPERT.getLandAccess(gameState, cce);
		let isEnemy = gameState.isPlayerEnemy(cce.owner());
		for (let cc of ccEnts)
		{
			if (cc.owner() != PlayerID)
				continue;
			if (PETRA_EXPERT.getLandAccess(gameState, cc) != access)
				continue;
 ...
 */
PETRA_EXPERT.HQ.prototype.trainEmergencyUnits = function(gameState, positions)
{
	...
	for (let pos of positions)
	{
		...
		for (const base of this.baseManagers())
		{
			if (!base.anchor || !base.anchor.position())
				continue;
			if (PETRA_EXPERT.getLandAccess(gameState, base.anchor) != access)
				continue;
	...
	const metadata = { "role": PETRA_EXPERT.Worker.ROLE_WORKER, "base": nearestAnchor.getMetadata(PlayerID, "base"), "plan": -1, "trainer": nearestAnchor.id() };
	if (autogarrison)
		metadata.garrisonType = PETRA_EXPERT.GarrisonManager.TYPE_PROTECTION;
	gameState.ai.queues.emergency.addPlan(new PETRA_EXPERT.TrainingPlan(gameState, templateFound[0], metadata, 2, 2));
	return true;
};

PETRA_EXPERT.HQ.prototype.canBuild = function(gameState, structure)
{
	...
	{
		this.buildManager.setUnbuildable(gameState, type, 10, "requirements");
		return false;
	}

	if (!this.buildManager.hasBuilder(type))
	{
		this.buildManager.setUnbuildable(gameState, type, 15, "nobuilder");
		return false;
	}
	...
	if (!this.hasActiveBase())
	{
		...
		{
			this.buildManager.setUnbuildable(gameState, type, 45, "room");
			return false;
		}
	}
	...
	{
		this.buildManager.setUnbuildable(gameState, type, 20, "limit");
		return false;
	}
...
PETRA_EXPERT.HQ.prototype.updateTerritories = function(gameState)
{
	...
	for (let j = 0; j < this.territoryMap.length; ++j)
	{
		if (this.borderMap.map[j] & PETRA_EXPERT.outside_Mask)
			continue;
		if (this.borderMap.map[j] & PETRA_EXPERT.fullFrontier_Mask)
			this.borderMap.map[j] &= ~PETRA_EXPERT.fullFrontier_Mask;	// reset the frontier
		...
		else
		{
			...
			for (let a of around)
			{
				...
				if (jz < 0 || jz >= width)
					continue;
				if (this.borderMap.map[jx+width*jz] & PETRA_EXPERT.outside_Mask)
					continue;
				...
				{
					this.borderMap.map[j] |= PETRA_EXPERT.narrowFrontier_Mask;
					break;
				}
			...
			if (onFrontier && !(this.borderMap.map[j] & PETRA_EXPERT.narrowFrontier_Mask))
				this.borderMap.map[j] |= PETRA_EXPERT.largeFrontier_Mask;
 ...
 */
PETRA_EXPERT.HQ.prototype.getBaseByID = function(baseID)
{
 ...
 */
PETRA_EXPERT.HQ.prototype.numActiveBases = function()
{
	return this.basesManager.numActiveBases();
};

PETRA_EXPERT.HQ.prototype.hasActiveBase = function()
{
	return this.basesManager.hasActiveBase();
};

PETRA_EXPERT.HQ.prototype.numPotentialBases = function()
{
	return this.basesManager.numPotentialBases();
};

PETRA_EXPERT.HQ.prototype.hasPotentialBase = function()
{
	return this.basesManager.hasPotentialBase();
};

PETRA_EXPERT.HQ.prototype.isDangerousLocation = function(gameState, pos, radius)
{
...
PETRA_EXPERT.HQ.prototype.isNearInvadingArmy = function(pos)
{
...
PETRA_EXPERT.HQ.prototype.isUnderEnemyFire = function(gameState, pos, radius = 0)
{
...
PETRA_EXPERT.HQ.prototype.updateCaptureStrength = function(gameState)
{
	...
	for (let ent of gameState.getOwnUnits().values())
	{
		...
		if (!this.capturableTargets.has(targetId))
			this.capturableTargets.set(targetId, {
				"strength": ent.captureStrength() * PETRA_EXPERT.getAttackBonus(ent, target, "Capture"),
				"ents": new Set([ent.id()])
			});
		else
		{
			let capturableTarget = this.capturableTargets.get(target.id());
			capturableTarget.strength += ent.captureStrength() * PETRA_EXPERT.getAttackBonus(ent, target, "Capture");
	...
	for (let [targetId, capturableTarget] of this.capturableTargets)
	{
		...
		for (let entId of capturableTarget.ents)
		{
			...
			if (allowCapture === undefined)
				allowCapture = PETRA_EXPERT.allowCapture(gameState, ent, target);
			let orderData = ent.unitAIOrderData();
 ...
 */
PETRA_EXPERT.HQ.prototype.isDefendable = function(ent)
{
 ...
 */
PETRA_EXPERT.HQ.prototype.getAccountedPopulation = function(gameState)
{
 ...
 */
PETRA_EXPERT.HQ.prototype.getAccountedWorkers = function(gameState)
{
	if (this.turnCache.accountedWorkers == undefined)
	{
		let workers = gameState.getOwnEntitiesByRole(PETRA_EXPERT.Worker.ROLE_WORKER, true).length;
		for (let ent of gameState.getOwnTrainingFacilities().values())
		{
			for (let item of ent.trainingQueue())
			{
...
PETRA_EXPERT.HQ.prototype.baseManagers = function()
{
 ...
 */
PETRA_EXPERT.HQ.prototype.baseAtIndex = function(territoryIndex)
{
 ...
 */
PETRA_EXPERT.HQ.prototype.update = function(gameState, queues, events)
{
	...
	*/
	...
	if (this.hasActiveBase())
	{
		...
		if (gameState.ai.playedTurn % 4 == 0)
		 	this.trainChampionInfantryMelee(gameState, queues);

		if (gameState.ai.playedTurn % 4 == 0)
			this.trainSupport(gameState, queues);

		// if (gameState.ai.playedTurn % 4 == 0)
		// 	this.trainVillager(gameState, queues);


		if (gameState.ai.playedTurn % 4 == 0)
			this.trainCitizenSoldiersMelee(gameState, queues);

		//if (gameState.ai.playedTurn % 4 == 0)
			//this.trainCitizenSoldiersRanged(gameState, queues);

		if (gameState.ai.playedTurn % 4 == 0)
			this.trainCavalry(gameState, queues);

		if (gameState.ai.playedTurn % 4 == 0)
			this.trainSiege(gameState, queues);

		if (gameState.ai.playedTurn % 4 == 0)
			this.trainElephants(gameState, queues);

		if (gameState.ai.playedTurn % 4 == 0)
			this.trainHealer(gameState, queues);

		// if (gameState.ai.playedTurn % 4 == 0)
		// 	this.trainHero(gameState, queues);
		...
		// if (gameState.ai.playedTurn % 4 == 0)
		// 	this.trainChampionInfantryRanged(gameState, queues);

		// if (gameState.ai.playedTurn % 4 == 0)
		// 	this.trainChampionCavalryMelee(gameState, queues);

		// if (gameState.ai.playedTurn % 4 == 0)
		// 	this.trainChampionCavalryRanged(gameState, queues);
	...
	if (gameState.ai.playedTurn % 3 == 0)
	{
		...
		if (this.Config.difficulty > PETRA_EXPERT.DIFFICULTY_SANDBOX)
	...
	if (this.Config.difficulty > PETRA_EXPERT.DIFFICULTY_SANDBOX && (this.hasActiveBase() || !this.canBuildUnits))
...
PETRA_EXPERT.HQ.prototype.Serialize = function()
{
	let properties = {
		"phasing": this.phasing,
...
PETRA_EXPERT.HQ.prototype.Deserialize = function(gameState, data)
{
