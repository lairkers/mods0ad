 ...
 */
PETRA_EXPERT.AttackPlan = function(gameState, Config, uniqueID, type = PETRA_EXPERT.AttackPlan.TYPE_DEFAULT, data)
{
	this.Config = Config;
	this.name = uniqueID;
	this.type = type;
	this.state = PETRA_EXPERT.AttackPlan.STATE_UNEXECUTED;
	...
	for (const base of gameState.ai.HQ.baseManagers())
	{
		if (!base.anchor || !base.anchor.position())
			continue;
		let access = PETRA_EXPERT.getLandAccess(gameState, base.anchor);
		if (!rallyPoint)
		{
	...
	if (gameState.ai.HQ.navalMap)
	{
		for (let structure of gameState.getEnemyStructures().values())
		{
			...
			if (!structure.position())
				continue;
			let access = PETRA_EXPERT.getLandAccess(gameState, structure);
			...
			else if (!this.overseas)
			{
				...
				if (!sea)
				{
					if (this.target)
					{
						API3.warn("Petra Expert: " + this.type + " " + this.name + " has an inaccessible target " +
	...
	let randBatch;
	if (currentPhaseIndex == 1)
		randBatch = pickRandom([1, 2]);
	else if (currentPhaseIndex == 2)
		randBatch = pickRandom([1, 2, 3]);
	else if (currentPhaseIndex >= 3)
		randBatch = pickRandom([1, 2, 3]);


	// neededShips is the minimal number of ships which should be available for transport
	if (type === PETRA_EXPERT.AttackPlan.TYPE_RUSH)
	{
		priority = 350;
		// if (currentPhaseIndex == 1)
		// 	priority = 500;
		...
		// else if (currentPhaseIndex >= 3)
		// 	priority = 750;
		 this.unitStat.Infantry = { "priority": 1.3, "minSize": 10, "targetSize": 45, "batchSize": randBatch, "classes": ["Infantry"],
		 	"interests": [["strength", 1]] };
		this.unitStat.FastMoving = { "priority": 1.3, "minSize": 2, "targetSize": 20, "batchSize": randBatch, "classes": ["FastMoving+CitizenSoldier"],
			"interests": [["strength", 1]] };

		//this.unitStat.ChampRangedInfantry = { "priority": 1.4, "minSize": 0, "targetSize": 15, "batchSize": randBatch, "classes": ["Infantry+Ranged+Champion"],
			//"interests": [["strength", 3]] };
		this.unitStat.ChampMeleeInfantry = { "priority": 1.5, "minSize": 0, "targetSize": 15, "batchSize": randBatch, "classes": ["Infantry+Melee+Champion"],
			"interests": [["strength", 3]] };

		this.unitStat.Dog = { "priority": 1.5, "minSize": 0, "targetSize": 10, "batchSize": 1, "classes": ["Dog+Melee"],
			"interests": [["strength", 3]] };
		this.unitStat.Hero = { "priority": 1.5, "minSize": 0, "targetSize": 1, "batchSize": 1, "classes": ["Hero"],
			"interests": [["strength", 2]] };
		this.unitStat.Healer = { "priority": 1.5, "minSize": 0, "targetSize": 5, "batchSize": 1, "classes": ["Healer"],
			"interests": [["strength", 2]] };
		if (data && data.targetSize)
			this.unitStat.Infantry.targetSize = data.targetSize;
		this.neededShips = 7;
	}
	else if (type === PETRA_EXPERT.AttackPlan.TYPE_RAID)
	{
		priority = 200;
		this.unitStat.FastMoving = { "priority": 1.5, "minSize": 3, "targetSize": 10, "batchSize": randBatch, "classes": ["FastMoving+CitizenSoldier"],
			"interests": [ ["strength", 1] ] };
		this.neededShips = 2;
	}
	else if (type === PETRA_EXPERT.AttackPlan.TYPE_HUGE_ATTACK)
	{
		priority = 250;
		// if (currentPhaseIndex == 1)
		// 	priority = 500;
		...
		this.unitStat.RangedInfantry = { "priority": 1.5, "minSize": 0, "targetSize": 15, "batchSize": randBatch, "classes": ["Infantry+Ranged+CitizenSoldier"],
			"interests": [["strength", 3]] };
		this.unitStat.MeleeInfantry = { "priority": 1.5, "minSize": 0, "targetSize": 15, "batchSize": randBatch, "classes": ["Infantry+Melee+CitizenSoldier"],
			"interests": [["strength", 3]] };
		this.unitStat.ChampRangedInfantry = { "priority": 1.5, "minSize": 0, "targetSize": 10, "batchSize": randBatch, "classes": ["Infantry+Ranged+Champion"],
			"interests": [["strength", 3]] };
		this.unitStat.ChampMeleeInfantry = { "priority": 1.5, "minSize": 0, "targetSize": 10, "batchSize": randBatch, "classes": ["Infantry+Melee+Champion"],
			"interests": [["strength", 3]] };
		this.unitStat.RangedFastMoving = { "priority": 1.5, "minSize": 0, "targetSize": 10, "batchSize": randBatch, "classes": ["FastMoving+Ranged+CitizenSoldier"],
			"interests": [["strength", 2]] };
		this.unitStat.MeleeFastMoving = { "priority": 1.5, "minSize": 0, "targetSize": 10, "batchSize": randBatch, "classes": ["FastMoving+Melee+CitizenSoldier"],
			"interests": [["strength", 2]] };
		this.unitStat.ChampRangedFastMoving = { "priority": 1.5, "minSize": 0, "targetSize": 10, "batchSize": randBatch, "classes": ["FastMoving+Ranged+Champion"],
			"interests": [["strength", 3]] };
		this.unitStat.ChampMeleeFastMoving = { "priority": 1.5, "minSize": 0, "targetSize": 10, "batchSize": randBatch, "classes": ["FastMoving+Melee+Champion"],
			"interests": [["strength", 2]] };
		this.unitStat.Hero = { "priority": 1.5, "minSize": 0, "targetSize": 1, "batchSize": 1, "classes": ["Hero"],
			"interests": [["strength", 2]] };
		this.unitStat.Dog = { "priority": 1.5, "minSize": 0, "targetSize": 10, "batchSize": 1, "classes": ["Dog+Melee"],
	...
	{
		priority = 200;
		this.unitStat.RangedInfantry = { "priority": 1.5, "minSize": 0, "targetSize": 20, "batchSize": randBatch, "classes": ["Infantry+Ranged"],
			"interests": [["canGather", 1], ["strength", 1.6], ["costsResource", 0.3, "stone"], ["costsResource", 0.3, "metal"]] };
		this.unitStat.MeleeInfantry = { "priority": 1.5, "minSize": 0, "targetSize": 20, "batchSize": randBatch, "classes": ["Infantry+Melee"],
			"interests": [["canGather", 1], ["strength", 1.6], ["costsResource", 0.3, "stone"], ["costsResource", 0.3, "metal"]] };
		//this.unitStat.FastMoving = { "priority": 1.5, "minSize": 0, "targetSize": 15, "batchSize": 2, "classes": ["FastMoving+CitizenSoldier"],
			//"interests": [["strength", 1]] };
	...
	else if (this.Config.difficulty < PETRA_EXPERT.DIFFICULTY_MEDIUM)
	{
		priority *= 0.8;
		variation *= 0.6;
	}
	...
	gameState.ai.queueManager.addQueue("plan_" + this.name +"_siege", priority+2);
	...
	this.siegeState = PETRA_EXPERT.AttackPlan.SIEGE_NOT_TESTED;
...
PETRA_EXPERT.AttackPlan.PREPARATION_FAILED = 0;
PETRA_EXPERT.AttackPlan.PREPARATION_KEEP_GOING = 1;
PETRA_EXPERT.AttackPlan.PREPARATION_START = 2;

PETRA_EXPERT.AttackPlan.SIEGE_NOT_TESTED = 0;
PETRA_EXPERT.AttackPlan.SIEGE_NO_TRAINER = 1;

/**
 * Siege added in build orders
 */
PETRA_EXPERT.AttackPlan.SIEGE_ADDED = 2;

PETRA_EXPERT.AttackPlan.STATE_UNEXECUTED = "unexecuted";
PETRA_EXPERT.AttackPlan.STATE_COMPLETING = "completing";
PETRA_EXPERT.AttackPlan.STATE_ARRIVED = "arrived";

PETRA_EXPERT.AttackPlan.TYPE_DEFAULT = "Attack";
PETRA_EXPERT.AttackPlan.TYPE_HUGE_ATTACK = "HugeAttack";
PETRA_EXPERT.AttackPlan.TYPE_RAID = "Raid";
PETRA_EXPERT.AttackPlan.TYPE_RUSH = "Rush";

PETRA_EXPERT.AttackPlan.prototype.init = function(gameState)
{
...
PETRA_EXPERT.AttackPlan.prototype.getName = function()
{
	return this.name;
};

PETRA_EXPERT.AttackPlan.prototype.getType = function()
{
	return this.type;
};

PETRA_EXPERT.AttackPlan.prototype.isStarted = function()
{
	return this.state !== PETRA_EXPERT.AttackPlan.STATE_UNEXECUTED && this.state !== PETRA_EXPERT.AttackPlan.STATE_COMPLETING;
};

PETRA_EXPERT.AttackPlan.prototype.isPaused = function()
{
	return this.paused;
};

PETRA_EXPERT.AttackPlan.prototype.setPaused = function(boolValue)
{
 ...
 */
PETRA_EXPERT.AttackPlan.prototype.canStart = function()
{
	if (!this.canBuildUnits)
		return true;
...
PETRA_EXPERT.AttackPlan.prototype.mustStart = function()
{
	if (this.isPaused())
		return false;
	...
	if (MinReachedEverywhere)
		return this.type === PETRA_EXPERT.AttackPlan.TYPE_RAID && this.target && this.target.foundationProgress() &&
		                                             this.target.foundationProgress() > 50;
	return false;
};

PETRA_EXPERT.AttackPlan.prototype.forceStart = function()
{
	for (let unitCat in this.unitStat)
	{
		let Unit = this.unitStat[unitCat];
...
PETRA_EXPERT.AttackPlan.prototype.emptyQueues = function()
{
	this.queue.empty();
...
PETRA_EXPERT.AttackPlan.prototype.removeQueues = function(gameState)
{
...
PETRA_EXPERT.AttackPlan.prototype.addBuildOrder = function(gameState, name, unitStats, resetQueue)
{
	if (!this.isStarted())
	{
...
PETRA_EXPERT.AttackPlan.prototype.addSiegeUnits = function(gameState)
{
	if (this.siegeState === PETRA_EXPERT.AttackPlan.SIEGE_ADDED || this.state !== PETRA_EXPERT.AttackPlan.STATE_UNEXECUTED)
		return false;
	...
	{
		this.siegeState = PETRA_EXPERT.AttackPlan.SIEGE_NO_TRAINER;
		return false;
	}
	...
	this.siegeState = PETRA_EXPERT.AttackPlan.SIEGE_ADDED;
	let targetSize;
	if (this.Config.difficulty < PETRA_EXPERT.DIFFICULTY_MEDIUM)
		targetSize = this.type === PETRA_EXPERT.AttackPlan.TYPE_HUGE_ATTACK ? Math.max(this.Config.difficulty, 1) : Math.max(this.Config.difficulty - 1, 1);
	else
		targetSize = this.type === PETRA_EXPERT.AttackPlan.TYPE_HUGE_ATTACK ? this.Config.difficulty + 1 : 1;
	targetSize = Math.max(Math.round(this.Config.popScaling * targetSize), this.type === PETRA_EXPERT.AttackPlan.TYPE_HUGE_ATTACK ? 1 : 0);
	if (!targetSize)
		return true;
	// no minsize as we don't want the plan to fail at the last minute though.
	let stat = { "priority": 1.45, "minSize": 0, "targetSize": targetSize, "batchSize": Math.min(targetSize, 1),
...
PETRA_EXPERT.AttackPlan.prototype.updatePreparation = function(gameState)
{
	...
	if (this.state === PETRA_EXPERT.AttackPlan.STATE_COMPLETING)
	{
		...
		else
		{
			...
			if (this.waitingForTransport())
				return PETRA_EXPERT.AttackPlan.PREPARATION_KEEP_GOING;
			...
			if (gameState.ai.elapsedTime < this.maxCompletingTime && this.hasForceOrder())
				return PETRA_EXPERT.AttackPlan.PREPARATION_KEEP_GOING;
			return PETRA_EXPERT.AttackPlan.PREPARATION_START;
		}
	}
	...
	if (this.type !== PETRA_EXPERT.AttackPlan.TYPE_RAID || !this.forced)    // Forced Raids have special purposes (as relic capture)
		this.assignUnits(gameState);
	if (this.type !== PETRA_EXPERT.AttackPlan.TYPE_RAID && gameState.ai.HQ.attackManager.getAttackInPreparation(PETRA_EXPERT.AttackPlan.TYPE_RAID) !== undefined)
	...
	if (gameState.ai.playedTurn % 5 == 0 && this.hasSiegeUnits())
	{
		...
		if (hasEnemies && this.unitCollection.length > 30 + totEnemies)
			this.forceStart();
	}
	let totEnemies = gameState.getEnemyUnits.length;
	// special case: if we've reached max pop, and we can start the plan, start it.
	/////////////////////////////TEST
	if (gameState.getPopulationMax() - gameState.getPopulation() < 15 || gameState.getPopulation() > 140 && this.unitCollection.length / totEnemies > 1.2)
	//if (gameState.getPopulation() > 135)
	{
		...
		else    // Instead of aborting, keep trying to build units
		{
			if (this.Config.debug > 1)
			{
				let am = gameState.ai.HQ.attackManager;
				API3.warn(" attacks upcoming: raid " + am.upcomingAttacks[PETRA_EXPERT.AttackPlan.TYPE_RAID].length +
					  " rush " + am.upcomingAttacks[PETRA_EXPERT.AttackPlan.TYPE_RUSH].length +
					  " attack " + am.upcomingAttacks[PETRA_EXPERT.AttackPlan.TYPE_DEFAULT].length +
					  " huge " + am.upcomingAttacks[PETRA_EXPERT.AttackPlan.TYPE_HUGE_ATTACK].length);
				API3.warn(" attacks started: raid " + am.startedAttacks[PETRA_EXPERT.AttackPlan.TYPE_RAID].length +
					  " rush " + am.startedAttacks[PETRA_EXPERT.AttackPlan.TYPE_RUSH].length +
					  " attack " + am.startedAttacks[PETRA_EXPERT.AttackPlan.TYPE_DEFAULT].length +
					  " huge " + am.startedAttacks[PETRA_EXPERT.AttackPlan.TYPE_HUGE_ATTACK].length);
			}
			// Continue training units instead of failing
			if (this.canBuildUnits)
			{
				// We still have time left to recruit units
				if (this.siegeState === PETRA_EXPERT.AttackPlan.SIEGE_NOT_TESTED ||
					this.siegeState === PETRA_EXPERT.AttackPlan.SIEGE_NO_TRAINER && gameState.ai.playedTurn % 5 == 0)
				...
				if (!this.buildOrders.length)
					return PETRA_EXPERT.AttackPlan.PREPARATION_FAILED;
			}
			return PETRA_EXPERT.AttackPlan.PREPARATION_KEEP_GOING;
		}
	}
	...
	else
	{
		if (this.canBuildUnits)
		{
			...
			// may happen if we have no more training facilities and build orders are canceled
			if (!this.buildOrders.length)
				return PETRA_EXPERT.AttackPlan.PREPARATION_FAILED;	// will abort the plan
		}
		return PETRA_EXPERT.AttackPlan.PREPARATION_KEEP_GOING;
	}

	// if we're here, it means we must start
	this.state = PETRA_EXPERT.AttackPlan.STATE_COMPLETING;
	...
	if (this.type === PETRA_EXPERT.AttackPlan.TYPE_RAID)
		this.maxCompletingTime = this.forced ? 0 : gameState.ai.elapsedTime + 20;
	else
	{
		if (this.type === PETRA_EXPERT.AttackPlan.TYPE_RUSH || this.forced)
			this.maxCompletingTime = gameState.ai.elapsedTime + 30;
		else
			this.maxCompletingTime = gameState.ai.elapsedTime + 45;
	...
	for (let ent of this.unitCollection.values())
	{
		...
		ent.setMetadata(PlayerID, "role", PETRA_EXPERT.Worker.ROLE_ATTACK);
		ent.setMetadata(PlayerID, "subrole", PETRA_EXPERT.Worker.SUBROLE_COMPLETING);
		let queued = false;
		if (ent.resourceCarrying() && ent.resourceCarrying().length)
			queued = PETRA_EXPERT.returnResources(gameState, ent);
		let index = PETRA_EXPERT.getLandAccess(gameState, ent);
		if (index == rallyIndex)
...
PETRA_EXPERT.AttackPlan.prototype.trainMoreUnits = function(gameState)
{
	...
	if (firstOrder[0] < firstOrder[3].targetSize)
	{
		...
		if (queue.length() <= 1)
		{
			...
			else
			{
				...
				data.role = gameState.getTemplate(template).hasClass("CitizenSoldier") ? PETRA_EXPERT.Worker.ROLE_WORKER : PETRA_EXPERT.Worker.ROLE_ATTACK;
				let trainingPlan = new PETRA_EXPERT.TrainingPlan(gameState, template, data, max, max);
...
PETRA_EXPERT.AttackPlan.prototype.assignUnits = function(gameState)
{
	let plan = this.name;
	let added = false;
	...
	for (let ent of gameState.getOwnUnits().filter(API3.Filters.byClass("Siege")).values())
	{
	...
	for (let ent of gameState.getOwnUnits().filter(API3.Filters.byClass("Healer")).values())
	{
		if (!this.isAvailableUnit(gameState, ent))
			continue;
		ent.setMetadata(PlayerID, "plan", plan);
	...
	for (let ent of gameState.getOwnUnits().filter(API3.Filters.byClass("Champion")).values())
	{
			if (!this.isAvailableUnit(gameState, ent))
				continue;
			ent.setMetadata(PlayerID, "plan", plan);
	...
	for (let ent of gameState.getOwnUnits().filter(API3.Filters.byClass("Hero")).values())
	{
		if (!this.isAvailableUnit(gameState, ent))
			continue;
		ent.setMetadata(PlayerID, "plan", plan);
	...
	for (let ent of gameState.getOwnUnits().filter(API3.Filters.byClass("Cavalry")).values())
		{
				if (!this.isAvailableUnit(gameState, ent))
					continue;
				ent.setMetadata(PlayerID, "plan", plan);
	...
	// let numberOfDefCitizenSoldiers = [];
	// for (let ent of gameState.getOwnEntitiesByRole(PETRA_EXPERT.Worker.ROLE_WORKER, true).values())
	// {
	// 		if (!ent.hasClass("CitizenSoldier") || !this.isAvailableUnit(gameState, ent))
	// 			continue;
	// 		numberOfDefCitizenSoldiers.push(ent);
	// }
	// if (numberOfDefCitizenSoldiers.length > 35)
	// {
	// 	const toMove = Math.min(10, numberOfDefCitizenSoldiers.length);
	// 	for (let i = 0; i < toMove; ++i)
	// 	{
	// 			const ent = numberOfDefCitizenSoldiers[i];
	// 			ent.setMetadata(PlayerID, "plan", plan);
	// 			this.unitCollection.updateEnt(ent);
	// 			added = true;
	// 	}
	...
	let num = 0;
	const numbase = {};
	let keep = this.type !== PETRA_EXPERT.AttackPlan.TYPE_RUSH ?
		6 + 4 * gameState.getNumPlayerEnemies() + 8 * this.Config.personality.defensive : 8;
	keep = Math.round(this.Config.popScaling * keep);
	for (const ent of gameState.getOwnEntitiesByRole(PETRA_EXPERT.Worker.ROLE_WORKER, true).values())
	{
		if (!ent.hasClass("CitizenSoldier") || !this.isAvailableUnit(gameState, ent))
			continue;
		...
		{
			API3.warn("Petra Expert problem ent without base ");
			PETRA_EXPERT.dumpEntity(ent);
			continue;
		}
		if (num++ < keep || numbase[baseID] < 5)
			continue;
		//if (this.type !== PETRA_EXPERT.AttackPlan.TYPE_RUSH && ent.getMetadata(PlayerID, "subrole") !== PETRA_EXPERT.Worker.SUBROLE_IDLE)
		if (ent.getMetadata(PlayerID, "subrole") !== PETRA_EXPERT.Worker.SUBROLE_IDLE)
			continue;
		ent.setMetadata(PlayerID, "plan", plan);
		this.unitCollection.updateEnt(ent);
		added = true;
...
PETRA_EXPERT.AttackPlan.prototype.isAvailableUnit = function(gameState, ent)
{
	if (!ent.position())
		return false;
...
PETRA_EXPERT.AttackPlan.prototype.reassignFastUnit = function(gameState)
{
	for (let ent of this.unitCollection.values())
	{
		...
		if (!ent.hasClasses(["FastMoving", "CitizenSoldier"]))
			continue;
		const raid = gameState.ai.HQ.attackManager.getAttackInPreparation(PETRA_EXPERT.AttackPlan.TYPE_RAID);
...
PETRA_EXPERT.AttackPlan.prototype.chooseTarget = function(gameState)
{
	...
	let targetIndex = PETRA_EXPERT.getLandAccess(gameState, this.target);
 ...
 */
PETRA_EXPERT.AttackPlan.prototype.getNearestTarget = function(gameState, position, sameLand)
{
	...
	else
	{
		...
		else if (this.type === PETRA_EXPERT.AttackPlan.TYPE_RUSH || this.type === PETRA_EXPERT.AttackPlan.TYPE_DEFAULT)
		{
	...
	for (let ent of targets.values())
	{
		...
		if (this.type !== PETRA_EXPERT.AttackPlan.TYPE_RUSH && this.type !== PETRA_EXPERT.AttackPlan.TYPE_RAID && ent.hasClass("Field"))
			dist += 100000;
		if (dist < minDist)
		{
			minDist = dist;
 ...
 */
PETRA_EXPERT.AttackPlan.prototype.defaultTargetFinder = function(gameState, playerEnemy)
{
...
PETRA_EXPERT.AttackPlan.prototype.isValidTarget = function(ent)
{
	if (!ent.position())
		return false;
	if (this.sameLand && PETRA_EXPERT.getLandAccess(this.gameState, ent) != this.sameLand)
		return false;
...
PETRA_EXPERT.AttackPlan.prototype.rushTargetFinder = function(gameState, playerEnemy)
{
	let targets = new API3.EntityCollection(gameState.sharedScript);
	let buildings;
	...
	if (!targets.hasEntities() && this.type === PETRA_EXPERT.AttackPlan.TYPE_RUSH && playerEnemy)
...
PETRA_EXPERT.AttackPlan.prototype.raidTargetFinder = function(gameState)
{
	let targets = new API3.EntityCollection(gameState.sharedScript);
 ...
 */
PETRA_EXPERT.AttackPlan.prototype.checkTargetObstruction = function(gameState, target, position)
{
	if (PETRA_EXPERT.getLandAccess(gameState, target) != gameState.ai.accessibility.getAccessValue(position))
		return target;
...
PETRA_EXPERT.AttackPlan.prototype.getPathToTarget = function(gameState, fixedRallyPoint = false)
{
	let startAccess = gameState.ai.accessibility.getAccessValue(this.rallyPoint);
	let endAccess = PETRA_EXPERT.getLandAccess(gameState, this.target);
...
PETRA_EXPERT.AttackPlan.prototype.setRallyPoint = function(gameState)
{
 ...
 */
PETRA_EXPERT.AttackPlan.prototype.StartAttack = function(gameState)
{
	if (this.Config.debug > 1)
	...
	for (let ent of this.unitCollection.values())
	{
		ent.setMetadata(PlayerID, "subrole", PETRA_EXPERT.Worker.SUBROLE_WALKING);
	...
	let targetAccess = PETRA_EXPERT.getLandAccess(gameState, this.target);
...
PETRA_EXPERT.AttackPlan.prototype.update = function(gameState, events)
{
	...
	if (this.state === PETRA_EXPERT.AttackPlan.STATE_ARRIVED)
	{
		...
		this.unitCollection.forEach(ent => {
			ent.stopMoving();
			ent.setMetadata(PlayerID, "subrole", PETRA_EXPERT.Worker.SUBROLE_ATTACKING);
		});
		if (this.type === PETRA_EXPERT.AttackPlan.TYPE_RUSH)   // try to find a better target for rush
		{
	...
	if (this.state == "")
	{
		...
		for (let evt of events.Attacked)
		{
			...
			if (PETRA_EXPERT.isSiegeUnit(ourUnit))
			{	// if our siege units are attacked, we'll send some units to deal with enemies.
				...
				{
					if (PETRA_EXPERT.isSiegeUnit(ent))	// needed as mauryan elephants are not filtered out
						continue;

					let allowCapture = PETRA_EXPERT.allowCapture(gameState, ent, attacker);
				...
				if (PETRA_EXPERT.isSiegeUnit(attacker) && attacker.hasClass("Melee") && ourUnit.hasClass("Melee") && ourUnit.canAttackTarget(attacker, PETRA_EXPERT.allowCapture(gameState, ourUnit, attacker)))
				{
					ourUnit.attack(attacker.id(), PETRA_EXPERT.allowCapture(gameState, ourUnit, attacker));
			...
			else
			{
				...
				else if (PETRA_EXPERT.isSiegeUnit(attacker))
				...
				else
				{
					...
					{
						let allowCapture = PETRA_EXPERT.allowCapture(gameState, ent, attacker);
						if (PETRA_EXPERT.isSiegeUnit(ent) || !ent.canAttackTarget(attacker, allowCapture))
							continue;
					...
					let allowCapture = PETRA_EXPERT.allowCapture(gameState, ourUnit, attacker);
		...
		for (let ent of this.unitCollection.values())
		{
			...
			{
				if (PETRA_EXPERT.isSiegeUnit(target) || target.hasClass("Hero"))
		...
		if (this.type === PETRA_EXPERT.AttackPlan.TYPE_RUSH)
		...
		for (let check = 0; check < lgth; check++)
		{
			...
			let siegeUnit = PETRA_EXPERT.isSiegeUnit(ent);
			if (ent.isIdle())
				needsUpdate = true;
			...
			if (this.isBlocked)
			{
				...
				range += 30;
			range *= range;
			let entAccess = PETRA_EXPERT.getLandAccess(gameState, ent);
			// Checking for gates if we're a siege unit.
			if (siegeUnit)
			{
				let mStruct = enemyStructures.filter(enemy => {
					if (!enemy.position() || !ent.canAttackTarget(enemy, PETRA_EXPERT.allowCapture(gameState, ent, enemy)))
						return false;
					if (API3.SquareVectorDistance(enemy.position(), ent.position()) > range)
						return false;
					if (enemy.foundationProgress() == 0)
						return false;
					if (PETRA_EXPERT.getLandAccess(gameState, enemy) != entAccess)
						return false;
					return true;
				}).toEntityArray();
				if (mStruct.length)
				{
					...
					if (mStruct[0].hasClass("Gate"))
						ent.attack(mStruct[0].id(), PETRA_EXPERT.allowCapture(gameState, ent, mStruct[0]));
					else
					{
						let rand = randIntExclusive(0, mStruct.length * 0.2);
						ent.attack(mStruct[rand].id(), PETRA_EXPERT.allowCapture(gameState, ent, mStruct[rand]));
					}
				}
				else
				{
			...
			else
			{
				...
				if (mUnit.length)
				{
					...
					ent.attack(mUnit[rand].id(), PETRA_EXPERT.allowCapture(gameState, ent, mUnit[rand]));
				}
					...
					else if (needsUpdate)  // really nothing   let's try to help our nearest unit
					{
						...
						this.unitCollection.forEach(unit => {
							...
							if (dist > distmin)
								return;
							distmin = dist;
							if (!ent.canAttackTarget(target, PETRA_EXPERT.allowCapture(gameState, ent, target)))
								return;
							attacker = target;
						});
						if (attacker)
							ent.attack(attacker.id(), PETRA_EXPERT.allowCapture(gameState, ent, attacker));
					}
				}
			}
		}
	...
	// for (let ent of this.unitCollection.values())
	// 	{
	// 		// check if unit is not attacking and does not have a combat order
	// 		let orderData = ent.unitAIOrderData();
	// 		let isAttacking = false;
	// 		if (orderData && orderData.length)
	// 		{
	// 			// if there is a target and it is an enemy — do not change
	// 			let targetId = orderData[0].target;
	// 			if (targetId)
	// 			{
	// 				let target = gameState.getEntityById(targetId);
	...
	// 			}
	// 		}
	// 		// if unit is not attacking, not moving and not busy — put it in idle
	// 		if (!isAttacking && 
	// 			(ent.getMetadata(PlayerID, "role") === PETRA_EXPERT.Worker.ROLE_ATTACK))
	// 		{
...
PETRA_EXPERT.AttackPlan.prototype.UpdateTransporting = function(gameState, events)
{
	let done = true;
	for (let ent of this.unitCollection.values())
	{
	...
	if (done)
	{
		this.state = PETRA_EXPERT.AttackPlan.STATE_ARRIVED;
		return;
	}
...
////////////////////////// TEST
PETRA_EXPERT.AttackPlan.prototype.checkEnemyProximity = function(gameState)
{
    //
    const detectionRange = 10000; // 100^2 -
    let enemyFound = false;
    let closestEnemy = null;
    let closestDistance = Infinity;
    
    //
    let ourPosition = this.position;
    if (!ourPosition)
        return;
    
    //
    let enemyUnits = gameState.getEnemyUnits();
    
    //
    for (let enemy of enemyUnits.values())
    {
        if (!enemy.position() || enemy.hasClass("Animal"))
            continue;
            
        let dist = API3.SquareVectorDistance(ourPosition, enemy.position());
    ...
    //
    if (enemyFound && closestEnemy)
    {
        //
        this.temporaryTarget = closestEnemy;
        ...
        for (let ent of this.unitCollection.values())
        {
            ...
            //
            let allowCapture = PETRA_EXPERT.allowCapture(gameState, ent, closestEnemy);
            if (ent.canAttackTarget(closestEnemy, allowCapture))
            {
                ent.attack(closestEnemy.id(), allowCapture);
                //
                ent.setMetadata(PlayerID, "temporaryTarget", true);
            }
        }
        
        //
        if (this.Config.debug > 1)
            API3.warn("Attack Plan " + this.type + " " + this.name + " encountered enemy units en route and is engaging.");
            
        return true;
    }
    
    //
    if (!enemyFound && this.temporaryTarget)
    {
        ...
        for (let ent of this.unitCollection.values())
        {
            if (!ent.position() || !ent.getMetadata(PlayerID, "temporaryTarget"))
                continue;
                
            ent.stopMoving();
            ent.setMetadata(PlayerID, "temporaryTarget", undefined);
        }
        ...
        //
        if (this.Config.debug > 1)
            API3.warn("Attack Plan " + this.type + " " + this.name + " continuing to main objective after engaging enemies.");
    }
    
    return false;
};

PETRA_EXPERT.AttackPlan.prototype.UpdateWalking = function(gameState, events)
{
	...
	this.checkEnemyProximity(gameState);
	for (let evt of events.Attacked)
	{
		if (!this.unitCollection.hasEntId(evt.target))
			continue;
...
PETRA_EXPERT.AttackPlan.prototype.UpdateTarget = function(gameState)
{
	...
	if (this.target && this.target.hasClass("Unit"))
	{
		...
		if (!this.targetPos)
		{
			let holder = PETRA_EXPERT.getHolder(gameState, this.target);
			...
			for (let attackType in attackManager.startedAttacks)
			{
				for (let attack of attackManager.startedAttacks[attackType])
				{
					...
					if (!attack.target || !gameState.getEntityById(attack.target.id()) ||
						...
						continue;
					if (accessIndex != PETRA_EXPERT.getLandAccess(gameState, attack.target))
						continue;
...
/** reset any units */
PETRA_EXPERT.AttackPlan.prototype.Abort = function(gameState)
{
	...
	if (this.unitCollection.hasEntities())
	{
		...
		for (let ent of this.unitCollection.values())
		{
			if (ent.getMetadata(PlayerID, "role") === PETRA_EXPERT.Worker.ROLE_ATTACK)
				ent.stopMoving();
			if (rallyPoint)
...
PETRA_EXPERT.AttackPlan.prototype.removeUnit = function(ent, update)
{
	if (ent.getMetadata(PlayerID, "role") === PETRA_EXPERT.Worker.ROLE_ATTACK)
	{
...
PETRA_EXPERT.AttackPlan.prototype.checkEvents = function(gameState, events)
{
	for (let evt of events.EntityRenamed)
	{
		if (!this.target || this.target.id() != evt.entity)
			continue;
		if (this.type === PETRA_EXPERT.AttackPlan.TYPE_RAID && !this.isStarted())
			this.target = undefined;
		else
	...
	if (!this.overseas || this.state !== PETRA_EXPERT.AttackPlan.STATE_UNEXECUTED)
		return;
...
PETRA_EXPERT.AttackPlan.prototype.waitingForTransport = function()
{
	for (let ent of this.unitCollection.values())
		if (ent.getMetadata(PlayerID, "transport") !== undefined)
			return true;
...
PETRA_EXPERT.AttackPlan.prototype.hasSiegeUnits = function()
{
	for (let ent of this.unitCollection.values())
		if (PETRA_EXPERT.isSiegeUnit(ent))
			return true;
	return false;
};

PETRA_EXPERT.AttackPlan.prototype.hasForceOrder = function(data, value)
{
	for (let ent of this.unitCollection.values())
	{
 ...
 */
PETRA_EXPERT.AttackPlan.prototype.getAttackAccess = function(gameState)
{
	for (let ent of this.unitCollection.filterNearest(this.position, 1).values())
		return PETRA_EXPERT.getLandAccess(gameState, ent);

	return 0;
};

PETRA_EXPERT.AttackPlan.prototype.debugAttack = function()
{
...
PETRA_EXPERT.AttackPlan.prototype.Serialize = function()
{
	let properties = {
		"name": this.name,
		"type": this.type,
...
PETRA_EXPERT.AttackPlan.prototype.Deserialize = function(gameState, data)
{
