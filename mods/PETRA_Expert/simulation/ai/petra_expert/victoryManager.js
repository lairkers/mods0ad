...
PETRA_EXPERT.VictoryManager = function(Config)
{
	this.Config = Config;
 ...
 */
PETRA_EXPERT.VictoryManager.prototype.init = function(gameState)
{
	...
	if (gameState.getVictoryConditions().has("regicide") || gameState.getVictoryConditions().has("conquest"))
	{
 ...
 */
PETRA_EXPERT.VictoryManager.prototype.checkEvents = function(gameState, events)
{
	if (gameState.getVictoryConditions().has("wonder"))
	{
	...
	if (gameState.getVictoryConditions().has("regicide") || gameState.getVictoryConditions().has("conquest"))
	{
		...
		for (let evt of events.TrainingFinished)
			for (let entId of evt.entities)
			{
				...
				if (ent && ent.isOwn(PlayerID) && ent.getMetadata(PlayerID, "role") === PETRA_EXPERT.Worker.ROLE_CRITICAL_ENT_HEALER)
	...
	for (let evt of events.UnGarrison)
	{
		...
		if ((ent.getMetadata(PlayerID, "role") === PETRA_EXPERT.Worker.ROLE_CRITICAL_ENT_HEALER ||
		     ent.getMetadata(PlayerID, "role") === PETRA_EXPERT.Worker.ROLE_CRITICAL_ENT_GUARD) && !this.guardEnts.get(evt.entity))
		{
...
PETRA_EXPERT.VictoryManager.prototype.removeCriticalEnt = function(gameState, criticalEntId)
{
 ...
 */
PETRA_EXPERT.VictoryManager.prototype.manageCriticalEntHealers = function(gameState, queues)
{
	...
	for (let data of this.criticalEnts.values())
	{
		...
		queues.healer.addPlan(new PETRA_EXPERT.TrainingPlan(gameState, template, { "role": PETRA_EXPERT.Worker.ROLE_CRITICAL_ENT_HEALER, "base": 0 }, 1, 1));
		return;
	}
};
 ...
 */
PETRA_EXPERT.VictoryManager.prototype.manageCriticalEntGuards = function(gameState)
{
	let numWorkers = gameState.getOwnEntitiesByRole(PETRA_EXPERT.Worker.ROLE_WORKER, true).length;
	if (numWorkers < 20)
	{
		for (let data of this.criticalEnts.values())
		{
			for (let guardId of data.guards.keys())
			{
				...
				if (!guardEnt || !guardEnt.hasClass("CitizenSoldier") ||
				    guardEnt.getMetadata(PlayerID, "role") !== PETRA_EXPERT.Worker.ROLE_CRITICAL_ENT_GUARD)
					continue;
...
PETRA_EXPERT.VictoryManager.prototype.tryAssignMilitaryGuard = function(gameState, guardEnt, criticalEnt, checkForSameAccess)
{
	if (guardEnt.getMetadata(PlayerID, "plan") !== undefined ||
	    ...
	    PETRA_EXPERT.getLandAccess(gameState, criticalEnt) != PETRA_EXPERT.getLandAccess(gameState, guardEnt)))
		return false;
	...
	guardEnt.setMetadata(PlayerID, "role", PETRA_EXPERT.Worker.ROLE_CRITICAL_ENT_GUARD);
	return true;
};

PETRA_EXPERT.VictoryManager.prototype.pickCriticalEntRetreatLocation = function(gameState, criticalEnt, emergency)
{
	...
	let accessIndex = PETRA_EXPERT.getLandAccess(gameState, criticalEnt);
	let bestBase = PETRA_EXPERT.getBestBase(gameState, criticalEnt, true);
 ...
 */
PETRA_EXPERT.VictoryManager.prototype.assignGuardToCriticalEnt = function(gameState, guardEnt, criticalEntId)
{
	...
	let guardEntAccess = PETRA_EXPERT.getLandAccess(gameState, guardEnt);
	let criticalEntAccess = PETRA_EXPERT.getLandAccess(gameState, criticalEnt);
	if (guardEntAccess == criticalEntAccess)
	{
		let queued = PETRA_EXPERT.returnResources(gameState, guardEnt);
		guardEnt.guard(criticalEnt, queued);
		const guardRole = guardEnt.getMetadata(PlayerID, "role") === PETRA_EXPERT.Worker.ROLE_CRITICAL_ENT_HEALER ? "healer" : "guard";
...
PETRA_EXPERT.VictoryManager.prototype.resetCaptureGaiaRelic = function(gameState)
{
...
PETRA_EXPERT.VictoryManager.prototype.update = function(gameState, events, queues)
{
	...
	if (gameState.ai.playedTurn % 10 != 0 ||
	    ...
	    !gameState.getVictoryConditions().has("capture_the_relic") && !gameState.getVictoryConditions().has("conquest"))
		return;
 ...
 */
PETRA_EXPERT.VictoryManager.prototype.captureGaiaRelic = function(gameState, relic)
{
	...
	for (let plan of plans)
	{
		...
		for (let ent of attack.unitCollection.values())
			capture += ent.captureStrength() * PETRA_EXPERT.getAttackBonus(ent, relic, "Capture");
	}
	// No need to make a new attack if already enough units
	if (capture > sumCapturePoints / 50)
		return;
	let relicPosition = relic.position();
	let access = PETRA_EXPERT.getLandAccess(gameState, relic);
	let units = gameState.getOwnUnits().filter(ent => {
		...
		{
			let attack = gameState.ai.HQ.attackManager.getPlan(plan);
			if (attack && (attack.state !== PETRA_EXPERT.AttackPlan.STATE_UNEXECUTED || attack.type === PETRA_EXPERT.AttackPlan.TYPE_RAID))
				return false;
		}
...
PETRA_EXPERT.VictoryManager.prototype.abortCaptureGaiaRelic = function(gameState, relicId)
{
...
PETRA_EXPERT.VictoryManager.prototype.Serialize = function()
{
	return {
...
PETRA_EXPERT.VictoryManager.prototype.Deserialize = function(data)
{
	for (let key in data)
		this[key] = data[key];
};
