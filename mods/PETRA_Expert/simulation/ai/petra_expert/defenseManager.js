PETRA_EXPERT.DefenseManager = function(Config)
{
...
PETRA_EXPERT.DefenseManager.prototype.update = function(gameState, events)
{
...
PETRA_EXPERT.DefenseManager.prototype.makeIntoArmy = function(gameState, entityID, type = "default")
{
	if (type == "default")
	{
	...
	this.armies.push(new PETRA_EXPERT.DefenseArmy(gameState, [entityID], type));
};

PETRA_EXPERT.DefenseManager.prototype.getArmy = function(partOfArmy)
{
	return this.armies.find(army => army.ID == partOfArmy);
};

PETRA_EXPERT.DefenseManager.prototype.isDangerous = function(gameState, entity)
{
	if (!entity.position())
		return false;
	...
	////////////////////////
	if (entity.hasClass("Worker") && gameState.isPlayerEnemy(entity.owner())) {
		// 
		let myBuildings = gameState.getOwnStructures();
		for (let building of myBuildings.values()) {
			if (API3.SquareVectorDistance(building.position(), entity.position()) < 30000) {
				this.targetList.push(entity.id());
				return true;
			}
		}
	}
...
PETRA_EXPERT.DefenseManager.prototype.checkEnemyUnits = function(gameState)
{
...
PETRA_EXPERT.DefenseManager.prototype.checkEnemyArmies = function(gameState)
{
...
PETRA_EXPERT.DefenseManager.prototype.assignDefenders = function(gameState)
{
	if (!this.armies.length)
		return;
	...
	for (let army of this.armies)
	{
		...
		for (let entId of army.foeEntities)
		{
			...
			if (!ent || !ent.position())
				continue;
			armyAccess = PETRA_EXPERT.getLandAccess(gameState, ent);
			break;
		}
		if (!armyAccess)
			API3.warn(" Petra Expert error: attacking army " + army.ID + " without access");
		army.recalculatePosition(gameState);
	...
	gameState.getOwnUnits().forEach(function(ent) {
		...
		if (ent.getMetadata(PlayerID, "plan") !== undefined && ent.getMetadata(PlayerID, "plan") != -1)
		{
			...
			if (subrole &&
				(subrole === PETRA_EXPERT.Worker.SUBROLE_COMPLETING || subrole === PETRA_EXPERT.Worker.SUBROLE_WALKING || subrole === PETRA_EXPERT.Worker.SUBROLE_ATTACKING))
				return;
		}
	...
	for (let ipass = 0; ipass < 2; ++ipass)
	{
		...
		for (let i = 0; i < potentialDefenders.length; ++i)
		{
			...
			let aMin;
			let distMin;
			let access = ipass == 0 ? PETRA_EXPERT.getLandAccess(gameState, ent) : undefined;
			for (let a = 0; a < armiesNeeding.length; ++a)
			{
				...
				if (!armiesNeeding[a].army.foeEntities.some(eEnt => {
					...
					return ent.canAttackTarget(eEntID, PETRA_EXPERT.allowCapture(gameState, ent, eEntID));
					}))
					continue;
			...
			armiesNeeding[aMin].need -= PETRA_EXPERT.getMaxStrength(ent, this.Config.debug, this.Config.DamageTypeImportance);
...
PETRA_EXPERT.DefenseManager.prototype.abortArmy = function(gameState, army)
{
	army.clear(gameState);
	for (let i = 0; i < this.armies.length; ++i)
	{
 ...
 */
PETRA_EXPERT.DefenseManager.prototype.checkEvents = function(gameState, events)
{
		...
		if (target.hasClass("Unit") && attacker.hasClass("Unit"))
		{
			...
			if (currentTarget)
			{
				...
				if (unitAIStateOrder === "REPAIR" && currentTarget.hasDefensiveFire())
					continue;
				if (unitAIStateOrder === "COMBAT" && !PETRA_EXPERT.isSiegeUnit(currentTarget) &&
				    gameState.ai.HQ.capturableTargets.has(orderData[0].target))
				{
					...
					for (let entId of capturableTarget.ents)
					{
						if (allAttacked[entId])
							continue;
						let ent = gameState.getEntityById(entId);
						if (!ent || !ent.position() || !ent.canAttackTarget(attacker, PETRA_EXPERT.allowCapture(gameState, ent, attacker)))
							continue;
					...
					if (minEnt)
					{
						...
						minEnt.attack(attacker.id(), PETRA_EXPERT.allowCapture(gameState, minEnt, attacker));
					}
				}
			}
			let allowCapture = PETRA_EXPERT.allowCapture(gameState, target, attacker);
...
PETRA_EXPERT.DefenseManager.prototype.garrisonUnitsInside = function(gameState, target, data)
{
	...
	let access = PETRA_EXPERT.getLandAccess(gameState, target);
	let garrisonManager = gameState.ai.HQ.garrisonManager;
	let garrisonArrowClasses = target.getGarrisonArrowClasses();
	const typeGarrison = data.type || PETRA_EXPERT.GarrisonManager.TYPE_PROTECTION;
	...
	if (allowMelee === undefined)
	{
		...
		if (data.attacker)
			allowMelee = data.attacker.hasClass("Structure") ? data.attacker.attackRange("Ranged") : !PETRA_EXPERT.isSiegeUnit(data.attacker);
		else
			allowMelee = true;
	}
	let units = gameState.getOwnUnits().filter(ent => {
		...
		if (!ent.hasClasses(garrisonArrowClasses))
			return false;
		if (typeGarrison !== PETRA_EXPERT.GarrisonManager.TYPE_DECAY && !allowMelee && ent.attackTypes().indexOf("Melee") != -1)
			return false;
		if (ent.getMetadata(PlayerID, "transport") !== undefined)
			return false;
		...
		if (ent.getMetadata(PlayerID, "plan") !== undefined && ent.getMetadata(PlayerID, "plan") >= 0)
		{
			...
			if (typeGarrison !== PETRA_EXPERT.GarrisonManager.TYPE_DECAY && subrole &&
				(subrole === PETRA_EXPERT.Worker.SUBROLE_COMPLETING || subrole === PETRA_EXPERT.Worker.SUBROLE_WALKING || subrole === PETRA_EXPERT.Worker.SUBROLE_ATTACKING))
				return false;
		}
		if (PETRA_EXPERT.getLandAccess(gameState, ent) != access)
			return false;
		return true;
...
PETRA_EXPERT.DefenseManager.prototype.garrisonSiegeUnit = function(gameState, unit)
{
	let distmin = Math.min();
	let nearest;
	let unitAccess = PETRA_EXPERT.getLandAccess(gameState, unit);
	let garrisonManager = gameState.ai.HQ.garrisonManager;
	for (let ent of gameState.getAllyStructures().values())
	{
		...
		if (ent.hitpoints() < ent.garrisonEjectHealth() * ent.maxHitpoints())
			continue;
		if (PETRA_EXPERT.getLandAccess(gameState, ent) != unitAccess)
			continue;
	...
	if (nearest)
		garrisonManager.garrison(gameState, unit, nearest, PETRA_EXPERT.GarrisonManager.TYPE_PROTECTION);
 ...
 */
PETRA_EXPERT.DefenseManager.prototype.garrisonAttackedUnit = function(gameState, unit, emergency = false)
{
	let distmin = Math.min();
	let nearest;
	let unitAccess = PETRA_EXPERT.getLandAccess(gameState, unit);
	...
	garrisonManager.garrison(gameState, unit, nearest, nearest.buffHeal() ? PETRA_EXPERT.GarrisonManager.TYPE_PROTECTION : PETRA_EXPERT.GarrisonManager.TYPE_EMERGENCY);
	return true;
};
 ...
 */
PETRA_EXPERT.DefenseManager.prototype.GetCooperationLevel = function(ally)
{
 ...
 */
PETRA_EXPERT.DefenseManager.prototype.switchToAttack = function(gameState, army)
{
	if (!army)
		return;
	for (let targetId of this.targetList)
	{
		...
		{
			let ent = gameState.getEntityById(entId);
			if (!ent || !ent.position() || PETRA_EXPERT.getLandAccess(gameState, ent) != targetAccess)
				continue;
...
PETRA_EXPERT.DefenseManager.prototype.Serialize = function()
{
	let properties = {
...
PETRA_EXPERT.DefenseManager.prototype.Deserialize = function(gameState, data)
{
	...
	{
		let army = new PETRA_EXPERT.DefenseArmy(gameState, []);
