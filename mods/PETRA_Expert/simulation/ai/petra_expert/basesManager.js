...
PETRA_EXPERT.BasesManager = function(Config)
{
	this.Config = Config;
...
PETRA_EXPERT.BasesManager.prototype.init = function(gameState)
{
	...
	this.noBase = new PETRA_EXPERT.BaseManager(gameState, this);
	this.noBase.init(gameState, PETRA_EXPERT.BaseManager.STATE_WITH_ANCHOR);
	...
	for (const cc of gameState.getOwnStructures().filter(API3.Filters.byClass("CivCentre")).values())
		if (cc.foundationProgress() === undefined)
			this.createBase(gameState, cc, PETRA_EXPERT.BaseManager.STATE_WITH_ANCHOR);
		else
			this.createBase(gameState, cc, PETRA_EXPERT.BaseManager.STATE_UNCONSTRUCTED);
};

/**
 * Initialization needed after deserialization (only called when deserialising).
 */
PETRA_EXPERT.BasesManager.prototype.postinit = function(gameState)
{
 ...
 */
PETRA_EXPERT.BasesManager.prototype.createBase = function(gameState, ent, type = PETRA_EXPERT.BaseManager.STATE_WITH_ANCHOR)
{
	const access = PETRA_EXPERT.getLandAccess(gameState, ent);
	let newbase;
	for (const base of this.baseManagers)
	{
		if (base.accessIndex != access)
			continue;
		if (type !== PETRA_EXPERT.BaseManager.STATE_ANCHORLESS && base.anchor)
			continue;
		if (type !== PETRA_EXPERT.BaseManager.STATE_ANCHORLESS)
		{
	...
	if (!newbase)
	{
		newbase = new PETRA_EXPERT.BaseManager(gameState, this);
...
PETRA_EXPERT.BasesManager.prototype.checkEvents = function(gameState, events)
{
	let addBase = false;
	...
	for (const evt of events.EntityRenamed)
	{
		...
		if (ent.getMetadata(PlayerID, "role") === PETRA_EXPERT.Worker.ROLE_WORKER)
		{
	...
	for (const evt of events.Create)
	{
		...
		if (ent.getMetadata(PlayerID, "base") == -1)	// Standard base around a cc
		{
			...
			const newbase = this.createBase(gameState, ent, PETRA_EXPERT.BaseManager.STATE_UNCONSTRUCTED);
		...
		{
			const newbase = this.createBase(gameState, ent, PETRA_EXPERT.BaseManager.STATE_ANCHORLESS);
			// Let's get a few units from other bases there to build this.
	...
	for (const evt of events.OwnershipChanged)
	{
		...
		{
			PETRA_EXPERT.getBestBase(gameState, ent).assignEntity(gameState, ent);
			continue;
		}
		if (ent.hasClass("CivCentre"))   // build a new base around it
		{
			...
			if (ent.foundationProgress() !== undefined)
				newbase = this.createBase(gameState, ent, PETRA_EXPERT.BaseManager.STATE_UNCONSTRUCTED);
			else
			{
				newbase = this.createBase(gameState, ent, PETRA_EXPERT.BaseManager.STATE_CAPTURED);
				addBase = true;
			}
		...
		else
		{
			...
			if (!ent.decaying() && ent.resourceDropsiteTypes())
				base = this.createBase(gameState, ent, PETRA_EXPERT.BaseManager.STATE_ANCHORLESS);
			else
				base = PETRA_EXPERT.getBestBase(gameState, ent) || this.noBase;
	...
	for (const evt of events.TrainingFinished)
	{
		for (const entId of evt.entities)
		{
			...
			if (ent.getMetadata(PlayerID, "role") === PETRA_EXPERT.Worker.ROLE_WORKER)
			{
				...
				if (ent.getMetadata(PlayerID, "base") === undefined)
				{
					base = PETRA_EXPERT.getBestBase(gameState, ent);
					base.assignEntity(gameState, ent);
				}
				else
			...
			else if (ent.resourceSupplyType() && ent.position())
			{
				...
				{
					if (!dropsite.position() || PETRA_EXPERT.getLandAccess(gameState, dropsite) != access)
						continue;
 ...
 */
PETRA_EXPERT.BasesManager.prototype.bulkPickWorkers = function(gameState, baseRef, number)
{
 ...
 */
PETRA_EXPERT.BasesManager.prototype.getTotalResourceLevel = function(gameState, resources = Resources.GetCodes(), proximity = ["nearby", "medium"])
{
	const total = {};
 ...
 */
PETRA_EXPERT.BasesManager.prototype.GetCurrentGatherRates = function(gameState)
{
...
PETRA_EXPERT.BasesManager.prototype.AddTCGatherer = function(supplyID)
{
...
PETRA_EXPERT.BasesManager.prototype.RemoveTCGatherer = function(supplyID)
{
...
PETRA_EXPERT.BasesManager.prototype.GetTCGatherer = function(supplyID)
{
	if (this.turnCache.resourceGatherer && this.turnCache.resourceGatherer[supplyID])
...
PETRA_EXPERT.BasesManager.prototype.AddTCResGatherer = function(resource)
{
...
PETRA_EXPERT.BasesManager.prototype.GetTCResGatherer = function(resource)
{
	const check = "resourceGatherer-" + resource;
	if (this.turnCache[check])
 ...
 */
PETRA_EXPERT.BasesManager.prototype.isResourceExhausted = function(resource)
{
 ...
 */
PETRA_EXPERT.BasesManager.prototype.numActiveBases = function()
{
...
PETRA_EXPERT.BasesManager.prototype.hasActiveBase = function()
{
	return !!this.numActiveBases();
};

PETRA_EXPERT.BasesManager.prototype.numPotentialBases = function()
{
	if (!this.turnCache.base)
		this.updateBaseCache();
...
PETRA_EXPERT.BasesManager.prototype.hasPotentialBase = function()
{
 ...
 */
PETRA_EXPERT.BasesManager.prototype.updateBaseCache = function()
{
...
PETRA_EXPERT.BasesManager.prototype.resetBaseCache = function()
{
	this.turnCache.base = undefined;
};

PETRA_EXPERT.BasesManager.prototype.baselessBase = function()
{
	return this.noBase;
};
 ...
 */
PETRA_EXPERT.BasesManager.prototype.getBaseByID = function(baseID)
{
 ...
 */
PETRA_EXPERT.BasesManager.prototype.assignGatherers = function()
{
	for (const base of this.baseManagers)
 ...
 */
PETRA_EXPERT.BasesManager.prototype.assignEntity = function(gameState, ent, territoryIndex)
{
	let bestbase;
	for (const base of this.baseManagers)
	{
	...
	if (!bestbase)	// entity outside our territory
	{
		if (ent.hasClass("Structure") && !ent.decaying() && ent.resourceDropsiteTypes())
			bestbase = this.createBase(gameState, ent, PETRA_EXPERT.BaseManager.STATE_ANCHORLESS);
		else
			bestbase = PETRA_EXPERT.getBestBase(gameState, ent) || this.noBase;
 ...
 */
PETRA_EXPERT.BasesManager.prototype.addGatherRates = function(gameState, rates)
{
	for (const base of this.baseManagers)
 ...
 */
PETRA_EXPERT.BasesManager.prototype.baseAtIndex = function(territoryIndex)
{
 ...
 * @param {number} territoryIndex
 */
PETRA_EXPERT.BasesManager.prototype.removeBaseFromTerritoryIndex = function(territoryIndex)
{
 ...
 */
PETRA_EXPERT.BasesManager.prototype.addTerritoryIndexToBase = function(gameState, territoryIndex, passabilityMap)
{
...
PETRA_EXPERT.BasesManager.prototype.reassignTerritories = function(deletedBase, territoryMap)
{
	...
	for (let j = 0; j < territoryMap.length; ++j)
	{
		...
		{
			API3.warn("Petra Expert reassignTerritories: should never happen");
 ...
 */
PETRA_EXPERT.BasesManager.prototype.update = function(gameState, queues, events)
{
...
PETRA_EXPERT.BasesManager.prototype.Serialize = function()
{
	const properties = {
...
PETRA_EXPERT.BasesManager.prototype.Deserialize = function(gameState, data)
{
	...
	for (const basedata of data.baseManagers)
	{
		...
		const newbase = new PETRA_EXPERT.BaseManager(gameState, this);
		newbase.Deserialize(gameState, basedata);
		newbase.init(gameState, PETRA_EXPERT.BaseManager.STATE_WITH_ANCHOR);
		newbase.Deserialize(gameState, basedata);
		this.baseManagers.push(newbase);
	}
};
