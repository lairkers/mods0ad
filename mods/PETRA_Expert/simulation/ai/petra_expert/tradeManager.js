 ...
 * Manage the trade
 */
PETRA_EXPERT.TradeManager = function(Config)
{
	this.Config = Config;
...
PETRA_EXPERT.TradeManager.prototype.init = function(gameState)
{
	this.traders = gameState.getOwnUnits().filter(API3.Filters.byMetadata(PlayerID, "role", PETRA_EXPERT.Worker.ROLE_TRADER));
	this.traders.registerUpdates();
	this.minimalGain = gameState.ai.HQ.navalMap ? 3 : 5;
};

PETRA_EXPERT.TradeManager.prototype.hasTradeRoute = function()
{
	return this.tradeRoute !== undefined;
};

PETRA_EXPERT.TradeManager.prototype.assignTrader = function(ent)
{
	ent.setMetadata(PlayerID, "role", PETRA_EXPERT.Worker.ROLE_TRADER);
	this.traders.updateEnt(ent);
};

PETRA_EXPERT.TradeManager.prototype.trainMoreTraders = function(gameState, queues)
{
	...
	gameState.getOwnTrainingFacilities().forEach(function(ent) {
		...
		{
			if (!item.metadata || !item.metadata.role || item.metadata.role !== PETRA_EXPERT.Worker.ROLE_TRADER)
				continue;
	...
	let template;
	const metadata = { "role": PETRA_EXPERT.Worker.ROLE_TRADER };
	if (this.tradeRoute.sea)
	{
		...
		gameState.ai.HQ.navalManager.seaTransportShips[this.tradeRoute.sea].forEach(function(ship) {
			if (already || !ship.hasClass("Trader"))
				return;
			if (ship.getMetadata(PlayerID, "role") === PETRA_EXPERT.Worker.ROLE_SWITCH_TO_TRADER)
			{
				already = true;
				return;
			}
		...
		if (shipToSwitch)
		{
			if (shipToSwitch.getMetadata(PlayerID, "transporter") === undefined)
				shipToSwitch.setMetadata(PlayerID, "role", PETRA_EXPERT.Worker.ROLE_TRADER);
			else
				shipToSwitch.setMetadata(PlayerID, "role", PETRA_EXPERT.Worker.ROLE_SWITCH_TO_TRADER);
			return;
		}
	...
	if (!gameState.getTemplate(template))
	{
		if (this.Config.debug > 0)
			API3.warn("Petra Expert error: trying to train " + template + " for civ " +
			          gameState.getPlayerCiv() + " but no template found.");
		return;
	}
	queues.trader.addPlan(new PETRA_EXPERT.TrainingPlan(gameState, template, metadata, 1, 1));
};

PETRA_EXPERT.TradeManager.prototype.updateTrader = function(gameState, ent)
{
	if (ent.hasClass("Ship") && gameState.ai.playedTurn % 5 == 0 &&
	    !ent.unitAIState().startsWith("INDIVIDUAL.COLLECTTREASURE") &&
	    PETRA_EXPERT.gatherTreasure(gameState, ent, true))
		return;
	...
	let access = ent.hasClass("Ship") ? PETRA_EXPERT.getSeaAccess(gameState, ent) : PETRA_EXPERT.getLandAccess(gameState, ent);
...
PETRA_EXPERT.TradeManager.prototype.setTradingGoods = function(gameState)
{
 ...
 */
PETRA_EXPERT.TradeManager.prototype.performBarter = function(gameState)
{
...
PETRA_EXPERT.TradeManager.prototype.checkEvents = function(gameState, events)
{
...
PETRA_EXPERT.TradeManager.prototype.activateProspection = function(gameState)
{
 ...
 */
PETRA_EXPERT.TradeManager.prototype.checkRoutes = function(gameState, accessIndex)
{
	...
	for (let m1 of market1.values())
	{
		if (!m1.position())
			continue;
		let access1 = PETRA_EXPERT.getLandAccess(gameState, m1);
		let sea1 = m1.hasClass("Naval") ? PETRA_EXPERT.getSeaAccess(gameState, m1) : undefined;
		for (let m2 of market2.values())
		{
			...
			if (!m2.position())
				continue;
			let access2 = PETRA_EXPERT.getLandAccess(gameState, m2);
			let sea2 = m2.hasClass("Naval") ? PETRA_EXPERT.getSeaAccess(gameState, m2) : undefined;
			let land = access1 == access2 ? access1 : undefined;
			let sea = sea1 && sea1 == sea2 ? sea1 : undefined;
			if (!land && !sea)
				continue;
			if (land && PETRA_EXPERT.isLineInsideEnemyTerritory(gameState, m1.position(), m2.position()))
				continue;
			let gainMultiplier;
	...
	if (this.Config.chat)
	{
		...
		{	// Warn an ally that we have a trade route with him
			PETRA_EXPERT.chatNewTradeRoute(gameState, owner);
...
PETRA_EXPERT.TradeManager.prototype.checkTrader = function(gameState, ent)
{
	    ...
	    possibleRoute.target.id() != presentRoute.source && possibleRoute.target.id() != presentRoute.target)
	{
		...
		{
			let closestBase = PETRA_EXPERT.getBestBase(gameState, ent, true);
...
PETRA_EXPERT.TradeManager.prototype.prospectForNewMarket = function(gameState, queues)
{
...
PETRA_EXPERT.TradeManager.prototype.isNewMarketWorth = function(expectedGain)
{
	if (!Resources.GetTradableCodes().length)
		return false;
...
PETRA_EXPERT.TradeManager.prototype.update = function(gameState, events, queues)
{
	if (gameState.ai.HQ.canBarter && Resources.GetBarterableCodes().length)
		this.performBarter(gameState);

	if (this.Config.difficulty <= PETRA_EXPERT.DIFFICULTY_VERY_EASY)
		return;
...
PETRA_EXPERT.TradeManager.prototype.routeEntToId = function(route)
{
	if (!route)
		return undefined;
...
PETRA_EXPERT.TradeManager.prototype.routeIdToEnt = function(gameState, route)
{
	if (!route)
		return undefined;
...
PETRA_EXPERT.TradeManager.prototype.Serialize = function()
{
	return {
...
PETRA_EXPERT.TradeManager.prototype.Deserialize = function(gameState, data)
{
	for (let key in data)
	{
