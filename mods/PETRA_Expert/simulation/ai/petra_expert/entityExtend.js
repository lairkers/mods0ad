...
PETRA_EXPERT.getMaxStrength = function(ent, debugLevel, DamageTypeImportance, againstClass)
{
	let strength = 0;
	...
	for (let type of attackTypes)
	{
		...
		for (let str in attackStrength)
		{
			...
			if (DamageTypeImportance[str])
				...
				API3.warn("Petra Expert: " + str + " unknown attackStrength in getMaxStrength (please add " + str + "  to config.js).");
		}
		...
		for (let str in attackTimes)
		{
			...
			case "prepare":
				...
				break;
			default:
				API3.warn("Petra Expert: " + str + " unknown attackTimes in getMaxStrength");
			}
		}
	}
	...
	if (resistanceStrength.Damage)
		for (let str in resistanceStrength.Damage)
		{
			...
			if (DamageTypeImportance[str])
				...
				API3.warn("Petra Expert: " + str + " unknown resistanceStrength in getMaxStrength (please add " + str + "  to config.js).");
		}
...
PETRA_EXPERT.getLandAccess = function(gameState, ent)
{
	if (ent.hasClass("Unit"))
	{
		...
		if (!pos)
		{
			let holder = PETRA_EXPERT.getHolder(gameState, ent);
			if (holder)
				return PETRA_EXPERT.getLandAccess(gameState, holder);

			API3.warn("Petra Expert error: entity without position, but not garrisoned");
...
PETRA_EXPERT.getSeaAccess = function(gameState, ent)
{
...
PETRA_EXPERT.setSeaAccess = function(gameState, ent)
{
	PETRA_EXPERT.getSeaAccess(gameState, ent);
};

/** Decide if we should try to capture (returns true) or destroy (return false) */
PETRA_EXPERT.allowCapture = function(gameState, ent, target)
{
	...
	{
		capture = ent.captureStrength() * PETRA_EXPERT.getAttackBonus(ent, target, "Capture");
	...
	else
	{
		...
		{
			capturable.strength += ent.captureStrength() * PETRA_EXPERT.getAttackBonus(ent, target, "Capture");
...
PETRA_EXPERT.getAttackBonus = function(ent, target, type)
{
	let attackBonus = 1;
...
PETRA_EXPERT.returnResources = function(gameState, ent)
{
	...
	for (let dropsite of dropsiteCollection.values())
	{
		...
		if (owner !== PlayerID && (!dropsite.isSharedDropsite() || !gameState.isPlayerMutualAlly(owner)))
			continue;
		if (PETRA_EXPERT.getLandAccess(gameState, dropsite) != access)
			continue;
...
PETRA_EXPERT.IsSupplyFull = function(gameState, ent)
{
 ...
 */
PETRA_EXPERT.getBestBase = function(gameState, ent, onlyConstructedBase = false, exclude = false)
{
	let pos = ent.position();
	let accessIndex;
	if (!pos)
	{
		...
		pos = holder.position();
		accessIndex = PETRA_EXPERT.getLandAccess(gameState, holder);
	}
	else
		accessIndex = PETRA_EXPERT.getLandAccess(gameState, ent);
...
PETRA_EXPERT.getHolder = function(gameState, ent)
{
...
PETRA_EXPERT.getBuiltEntity = function(gameState, ent)
{
 ...
 */
PETRA_EXPERT.isNotWorthBuilding = function(gameState, ent)
{
 ...
 */
PETRA_EXPERT.isLineInsideEnemyTerritory = function(gameState, pos1, pos2, step=70)
{
...
PETRA_EXPERT.gatherTreasure = function(gameState, ent, water = false)
{
	...
	let treasureFound;
	let distmin = Math.min();
	let access = water ? PETRA_EXPERT.getSeaAccess(gameState, ent) : PETRA_EXPERT.getLandAccess(gameState, ent);
	for (let treasure of gameState.ai.HQ.treasures.values())
	{
		...
		if (lastGathered && gameState.ai.elapsedTime - lastGathered < 20)
			continue;
		if (!water && access != PETRA_EXPERT.getLandAccess(gameState, treasure))
			continue;
		if (water && access != PETRA_EXPERT.getSeaAccess(gameState, treasure))
			continue;
...
PETRA_EXPERT.dumpEntity = function(ent)
{
	if (!ent)
		return;
