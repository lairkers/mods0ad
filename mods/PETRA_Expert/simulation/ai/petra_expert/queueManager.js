...
PETRA_EXPERT.QueueManager = function(Config, queues)
{
	this.Config = Config;
	this.queues = queues;
	this.priorities = {};
...
PETRA_EXPERT.QueueManager.prototype.getAvailableResources = function(gameState)
{
...
PETRA_EXPERT.QueueManager.prototype.getTotalAccountedResources = function()
{
...
PETRA_EXPERT.QueueManager.prototype.currentNeeds = function(gameState)
{
...
// TODO: many things.
PETRA_EXPERT.QueueManager.prototype.wantedGatherRates = function(gameState)
{
...
PETRA_EXPERT.QueueManager.prototype.printQueues = function(gameState)
{
    let numWorkers = 0;
    ...
    API3.warn("---------- QUEUES ------------ Population: " + gameState.getPopulation() + " Workers: " + numWorkers);
    ...
    for (let i in this.queues)
    {
        ...
        if (q.hasQueuedUnits())
        {
            API3.warn(i + ": (Priority " + this.priorities[i] + " Resources " + uneval(this.accounts[i]) + ")");
            API3.warn(" Max Account Wanted (0.6): " + uneval(q.maxAccountWanted(gameState, 0.6)));
        }
        
        // 
        for (let j = 0; j < q.plans.length; j++)
        {
            let plan = q.plans[j];
            let qStr = "     " + plan.type + " ";
            if (plan.number)
            ...
            // 
            let timeFrame = "";
            if (j === 0 && plan.isGo(gameState))
                timeFrame = "Short-term (30s)";
            else if (j === 1 && plan.isGo(gameState))
                timeFrame = "Medium-term (60s)";
            else
                timeFrame = "Long-term (300s)";
...
PETRA_EXPERT.QueueManager.prototype.clear = function()
{
	for (let i in this.queues)
		this.queues[i].empty();
};
 ...
 */
PETRA_EXPERT.QueueManager.prototype.setAccounts = function(gameState, cost, i)
{
 ...
 */
PETRA_EXPERT.QueueManager.prototype.transferAccounts = function(cost, i, j)
{
	for (let res of Resources.GetCodes())
	{
 ...
 */
PETRA_EXPERT.QueueManager.prototype.distributeResources = function(gameState)
{
	...
	for (let res of Resources.GetCodes())
	{
		...
		if (available < 0)
			API3.warn("Petra Expert: problem with remaining " + res + " in queueManager " + available);
	}
};
...
PETRA_EXPERT.QueueManager.prototype.switchResource = function(gameState, res)
{
    //
    for (let j = 0; j < this.queueArrays.length; ++j)
    {
        let queueName = this.queueArrays[j][0];
        ...
        if (this.accounts[queueName][res] >= queueCost[res])
            continue;

		//
		let sortedLowerQueues = this.queueArrays.slice(j + 1).sort((a, b) => this.priorities[a[0]] - this.priorities[b[0]]);

        //
		for (let i = 0; i < sortedLowerQueues.length; ++i)
        {
            let otherQueueName = sortedLowerQueues[i][0];
            let otherQueue = this.queues[otherQueueName];
            
            //
            // if (otherQueue.switched !== 0)
            //     continue;

            let available = this.accounts[otherQueueName][res];
            if (available <= 0)
                continue;

            let needed = queueCost[res] - this.accounts[queueName][res];
            let diff = Math.min(available, needed);
            
            if (diff > 0)
            {
                this.accounts[queueName][res] += diff;
                this.accounts[otherQueueName][res] -= diff;
                ++otherQueue.switched;

                if (this.Config.debug > 1)
                    API3.warn(`Reallocated ${diff} ${res} from ${otherQueueName} to ${queueName}`);
            }
        }
    }
};
...
// 				continue;
// 			if (this.priorities[i] >= this.priorities[j])
// 				continue;
...
PETRA_EXPERT.QueueManager.prototype.startNextItems = function(gameState)
{
	for (let q of this.queueArrays)
	{
		let name = q[0];
...
PETRA_EXPERT.QueueManager.prototype.update = function(gameState)
{
	...
	if (this.Config.debug > 1 && gameState.ai.playedTurn%10 === 0)
...
PETRA_EXPERT.QueueManager.prototype.checkPausedQueues = function(gameState)
{
	const numWorkers = gameState.countOwnEntitiesAndQueuedWithRole(PETRA_EXPERT.Worker.ROLE_WORKER);
	...
	for (let q in this.queues)
	{
		...
		if (!gameState.ai.HQ.hasPotentialBase())
			...
			toBePaused = q != "citizenSoldier" && q != "support" && q != "emergency";
		else if (numWorkers < workersMin * 2 / 3)
			toBePaused = q == "civilCentre" ||
...
PETRA_EXPERT.QueueManager.prototype.canAfford = function(queue, cost)
{
...
PETRA_EXPERT.QueueManager.prototype.pauseQueue = function(queue, scrapAccounts)
{
...
PETRA_EXPERT.QueueManager.prototype.unpauseQueue = function(queue)
{
	if (this.queues[queue])
		this.queues[queue].paused = false;
};

PETRA_EXPERT.QueueManager.prototype.pauseAll = function(scrapAccounts, but)
{
	for (let q in this.queues)
	{
		if (q == but)
...
PETRA_EXPERT.QueueManager.prototype.unpauseAll = function(but)
{
	for (let q in this.queues)
		if (q != but)
...
PETRA_EXPERT.QueueManager.prototype.addQueue = function(queueName, priority)
{
	if (this.queues[queueName] !== undefined)
		return;

	this.queues[queueName] = new PETRA_EXPERT.Queue();
...
PETRA_EXPERT.QueueManager.prototype.removeQueue = function(queueName)
{
	if (this.queues[queueName] === undefined)
		return;
...
PETRA_EXPERT.QueueManager.prototype.getPriority = function(queueName)
{
	return this.priorities[queueName];
};

PETRA_EXPERT.QueueManager.prototype.changePriority = function(queueName, newPriority)
{
	if (this.Config.debug > 1)
...
PETRA_EXPERT.QueueManager.prototype.Serialize = function()
{
	let accounts = {};
	let queues = {};
	for (let q in this.queues)
...
PETRA_EXPERT.QueueManager.prototype.Deserialize = function(gameState, data)
{
	...
	{
		this.queues[q] = new PETRA_EXPERT.Queue();
