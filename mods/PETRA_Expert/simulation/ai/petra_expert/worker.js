 ...
 */
PETRA_EXPERT.Worker = function(base)
{
	this.ent = undefined;
	this.base = base;
	this.baseID = base.ID;
...
PETRA_EXPERT.Worker.ROLE_ATTACK = "attack";
PETRA_EXPERT.Worker.ROLE_TRADER = "trader";
PETRA_EXPERT.Worker.ROLE_SWITCH_TO_TRADER = "switchToTrader";
PETRA_EXPERT.Worker.ROLE_WORKER = "worker";
PETRA_EXPERT.Worker.ROLE_CRITICAL_ENT_GUARD = "criticalEntGuard";
PETRA_EXPERT.Worker.ROLE_CRITICAL_ENT_HEALER = "criticalEntHealer";

PETRA_EXPERT.Worker.SUBROLE_DEFENDER = "defender";
PETRA_EXPERT.Worker.SUBROLE_IDLE = "idle";
PETRA_EXPERT.Worker.SUBROLE_BUILDER = "builder";
PETRA_EXPERT.Worker.SUBROLE_COMPLETING = "completing";
PETRA_EXPERT.Worker.SUBROLE_WALKING = "walking";
PETRA_EXPERT.Worker.SUBROLE_ATTACKING = "attacking";
PETRA_EXPERT.Worker.SUBROLE_GATHERER = "gatherer";
PETRA_EXPERT.Worker.SUBROLE_HUNTER = "hunter";
PETRA_EXPERT.Worker.SUBROLE_FISHER = "fisher";
PETRA_EXPERT.Worker.SUBROLE_GARRISONING = "garrisoning";

PETRA_EXPERT.Worker.prototype.update = function(gameState, ent)
{
	...
	if (ent.getMetadata(PlayerID, "transport") !== undefined)
	{
		...
		if (subrole === PETRA_EXPERT.Worker.SUBROLE_BUILDER && ent.getMetadata(PlayerID, "target-foundation") !== undefined)
		{
			...
			if (!target && plan && plan.state === PETRA_EXPERT.TransportPlan.BOARDING && ent.position())
				plan.removeUnit(gameState, ent);
		}
		// and gatherer if there are no more dropsite accessible in the base the ent is going to
		if (subrole === PETRA_EXPERT.Worker.SUBROLE_GATHERER || subrole === PETRA_EXPERT.Worker.SUBROLE_HUNTER)
		{
			let plan = gameState.ai.HQ.navalManager.getPlan(ent.getMetadata(PlayerID, "transport"));
			if (plan.state === PETRA_EXPERT.TransportPlan.BOARDING && ent.position())
			{
				...
				{
					if (PETRA_EXPERT.getLandAccess(gameState, structure) != plan.endIndex)
						continue;
					let resourceDropsiteTypes = PETRA_EXPERT.getBuiltEntity(gameState, structure).resourceDropsiteTypes();
				...
				if (!hasDropsite)
				{
					...
					{
						if (!unit.position() || PETRA_EXPERT.getLandAccess(gameState, unit) != plan.endIndex)
							continue;
	...
	this.entAccess = PETRA_EXPERT.getLandAccess(gameState, ent);
	...
	if ((subrole === PETRA_EXPERT.Worker.SUBROLE_HUNTER || subrole === PETRA_EXPERT.Worker.SUBROLE_GATHERER) &&
		...
		if (unitAIState == "INDIVIDUAL.COMBAT.APPROACHING" && ent.unitAIOrderData().length)
		{
			...
			if (orderData && orderData.target)
			{
				...
				if (target && target.resourceSupplyType() && target.resourceSupplyType().generic == "food")
				{
					...
					{
						let distanceSquare = PETRA_EXPERT.isFastMoving(ent) ? 90000 : 30000;
						...
						for (let dropsite of foodDropsites.values())
						{
							...
							if (owner != PlayerID && (!dropsite.isSharedDropsite() || !gameState.isPlayerMutualAlly(owner)))
								continue;
							if (targetAccess != PETRA_EXPERT.getLandAccess(gameState, dropsite))
								continue;
	...
	{
		if (subrole === PETRA_EXPERT.Worker.SUBROLE_FISHER)
			...
			!ent.getMetadata(PlayerID, "PartOfArmy"))
		{
			...
			{
				let target = gameState.getEntityById(orderData.target);
				if (target && (!target.position() || PETRA_EXPERT.getLandAccess(gameState, target) != this.entAccess))
				{
					if (this.retryWorking(gameState, subrole))
						return;
					ent.stopMoving();
			...
			!ent.getMetadata(PlayerID, "PartOfArmy"))
		{
			...
			if (orderData && orderData.target && orderData.attackType && orderData.attackType == "Capture")
			{
				...
				if (target && target.owner() > 0 && !gameState.isPlayerAlly(target.owner()))
					ent.attack(orderData.target, PETRA_EXPERT.allowCapture(gameState, ent, target));
			}
		}
		return;
	}
	...
	if (subrole === PETRA_EXPERT.Worker.SUBROLE_GATHERER)
	{
		if (ent.isIdle())
		{
			...
			else if (!PETRA_EXPERT.returnResources(gameState, ent))     // try to deposit resources
			{
		...
		else if (unitAIStateOrder == "GATHER")
		{
			...
			if (unitAIState == "INDIVIDUAL.GATHER.RETURNINGRESOURCE.APPROACHING")
			{
				if (gameState.ai.playedTurn % 10 == 0)
				{
					...
					if (dropsite && dropsite.position() && this.entAccess != PETRA_EXPERT.getLandAccess(gameState, dropsite))
						PETRA_EXPERT.returnResources(gameState, this.ent);
				}
	...
	else if (subrole === PETRA_EXPERT.Worker.SUBROLE_BUILDER)
	{
		...
		{
			let goalAccess = PETRA_EXPERT.getLandAccess(gameState, target);
			let queued = PETRA_EXPERT.returnResources(gameState, ent);
	...
	else if (subrole === PETRA_EXPERT.Worker.SUBROLE_HUNTER)
	{
		...
		else	// Perform some sanity checks
		{
			if (unitAIStateOrder == "GATHER")
			{
				...
				else if (unitAIState == "INDIVIDUAL.GATHER.RETURNINGRESOURCE.APPROACHING")
				{
					...
					let dropsite = gameState.getEntityById(ent.unitAIOrderData()[0].target);
					if (dropsite && dropsite.position() && this.entAccess != PETRA_EXPERT.getLandAccess(gameState, dropsite))
						PETRA_EXPERT.returnResources(gameState, ent);
				}
			}
		}
	}
	else if (subrole === PETRA_EXPERT.Worker.SUBROLE_FISHER)
	{
		if (ent.isIdle())
			this.startFishing(gameState);
...
PETRA_EXPERT.Worker.prototype.retryWorking = function(gameState, subrole)
{
	switch (subrole)
	{
	case PETRA_EXPERT.Worker.SUBROLE_GATHERER:
		return this.startGathering(gameState);
	case PETRA_EXPERT.Worker.SUBROLE_HUNTER:
		return this.startHunting(gameState);
	case PETRA_EXPERT.Worker.SUBROLE_FISHER:
		return this.startFishing(gameState);
	case PETRA_EXPERT.Worker.SUBROLE_BUILDER:
...
PETRA_EXPERT.Worker.prototype.startBuilding = function(gameState)
{
	let target = gameState.getEntityById(this.ent.getMetadata(PlayerID, "target-foundation"));
	if (!target || target.foundationProgress() === undefined && target.needsRepair() == false)
		return false;
	if (PETRA_EXPERT.getLandAccess(gameState, target) != this.entAccess)
		return false;
	this.ent.repair(target, target.hasClass("House"));  // autocontinue=true for houses
	return true;
};

PETRA_EXPERT.Worker.prototype.startGathering = function(gameState)
{
	// First look for possible treasure if any
	if (PETRA_EXPERT.gatherTreasure(gameState, this.ent))
		return true;
	...
	const findSupply = function(worker, supplies) {
		...
		for (let i = 0; i < supplies.length; ++i)
		{
			...
			if (PETRA_EXPERT.IsSupplyFull(gameState, supplies[i].ent))
				continue;
	...
	let shouldBuild = this.ent.isBuilder() && foundations.some(function(foundation) {
		if (!foundation || PETRA_EXPERT.getLandAccess(gameState, foundation) != this.entAccess)
			return false;
		...
		if (structure.resourceDropsiteTypes() && structure.resourceDropsiteTypes().indexOf(resource) != -1)
		{
			...
			this.ent.setMetadata(PlayerID, "subrole", PETRA_EXPERT.Worker.SUBROLE_BUILDER);
	...
	shouldBuild = this.ent.isBuilder() && foundations.some(function(foundation) {
		...
		if (structure.resourceDropsiteTypes() && structure.resourceDropsiteTypes().indexOf(resource) != -1)
		{
			let foundationAccess = PETRA_EXPERT.getLandAccess(gameState, foundation);
	...
	this.ent.setMetadata(PlayerID, "subrole", PETRA_EXPERT.Worker.SUBROLE_IDLE);
	return false;
};
 ...
 */
PETRA_EXPERT.Worker.prototype.startHunting = function(gameState, position)
{
	// First look for possible treasure if any
	if (!position && PETRA_EXPERT.gatherTreasure(gameState, this.ent))
		return true;
	...
	let isFastMoving = PETRA_EXPERT.isFastMoving(this.ent);
	...
	let hasFoodDropsiteWithinDistance = function(supplyPosition, supplyAccess, distSquare)
	{
		for (let dropsite of foodDropsites.values())
		{
			...
			if (owner != PlayerID && (!dropsite.isSharedDropsite() || !gameState.isPlayerMutualAlly(owner)))
				continue;
			if (supplyAccess != PETRA_EXPERT.getLandAccess(gameState, dropsite))
				continue;
	...
	for (let supply of resources.values())
	{
		...
		if (PETRA_EXPERT.IsSupplyFull(gameState, supply))
			continue;
		// Check if available resource is worth one additionnal gatherer (except for farms).
		...
		let supplyAccess = PETRA_EXPERT.getLandAccess(gameState, supply);
		...
		if (!isFastMoving && canFlee && territoryOwner == 0)
			continue;
		let distanceSquare = isFastMoving ? 150000 : (canFlee ? 70000 : 120000);
...
PETRA_EXPERT.Worker.prototype.startFishing = function(gameState)
{
	...
	let fisherSea = PETRA_EXPERT.getSeaAccess(gameState, this.ent);
	...
	let nearestDropsiteDist = function(supply) {
		...
		for (let dropsite of fishDropsites)
		{
			...
			if (owner != PlayerID && (!dropsite.isSharedDropsite() || !gameState.isPlayerMutualAlly(owner)))
				continue;
			if (fisherSea != PETRA_EXPERT.getSeaAccess(gameState, dropsite))
				continue;
	...
	if (this.ent.getMetadata(PlayerID, "subrole") === PETRA_EXPERT.Worker.SUBROLE_FISHER)
		this.ent.setMetadata(PlayerID, "subrole", PETRA_EXPERT.Worker.SUBROLE_IDLE);
	return false;
};

PETRA_EXPERT.Worker.prototype.gatherNearestField = function(gameState, baseID)
{
	...
	{
		if (PETRA_EXPERT.IsSupplyFull(gameState, field))
			continue;
 ...
 */
PETRA_EXPERT.Worker.prototype.buildAnyField = function(gameState, baseID)
{
 ...
 */
PETRA_EXPERT.Worker.prototype.moveToGatherer = function(gameState, ent, forced)
{
	...
	if (!forced && gameState.ai.elapsedTime < (ent.getMetadata(PlayerID, "nextMoveToGatherer") || 5))
		return;
	const gatherers = this.base.workersBySubrole(gameState, PETRA_EXPERT.Worker.SUBROLE_GATHERER);
	let dist = Math.min();
	let destination;
	...
	for (let gatherer of gatherers.values())
	{
		...
		if (!gatherer.position() || gatherer.getMetadata(PlayerID, "transport") !== undefined ||
		    PETRA_EXPERT.getLandAccess(gameState, gatherer) != access || gatherer.isIdle())
			continue;
 ...
 */
PETRA_EXPERT.Worker.prototype.isInaccessibleSupply = function(gameState)
{
