...
PETRA_EXPERT.GarrisonManager = function(Config)
{
	this.Config = Config;
...
PETRA_EXPERT.GarrisonManager.TYPE_FORCE = "force";
PETRA_EXPERT.GarrisonManager.TYPE_TRADE = "trade";
PETRA_EXPERT.GarrisonManager.TYPE_PROTECTION = "protection";
PETRA_EXPERT.GarrisonManager.TYPE_DECAY = "decay";
PETRA_EXPERT.GarrisonManager.TYPE_EMERGENCY = "emergency";


PETRA_EXPERT.GarrisonManager.TYPE_SUPPORT = "support";

PETRA_EXPERT.GarrisonManager.prototype.update = function(gameState, events)
{
	...
	for (let [id, data] of this.holders.entries())
	{
		...
		for (let j = 0; j < list.length; ++j)
		{
			...
			else
			{
				...
				else
				{
					...
					{
						API3.warn("Petra Expert garrison error: unit " + ent.id() + " (" + ent.genericName() +
		...
		if (gameState.ai.elapsedTime - holder.getMetadata(PlayerID, "holderTimeUpdate") > 3)
		{
			...
			for (let ent of gameState.getEnemyEntities().values())
			{
				...
				else if (PETRA_EXPERT.isSiegeUnit(ent))
				{
	...
	for (let [id, gmin] of this.decayingStructures.entries())
	{
		...
		if (!ent || ent.owner() !== PlayerID)
			...
			gameState.ai.HQ.defenseManager.garrisonUnitsInside(gameState, ent, { "min": gmin, "type": PETRA_EXPERT.GarrisonManager.TYPE_DECAY });
	}
};

/** TODO should add the units garrisoned inside garrisoned units */
PETRA_EXPERT.GarrisonManager.prototype.numberOfGarrisonedUnits = function(holder)
{
...
/** TODO should add the units garrisoned inside garrisoned units */
PETRA_EXPERT.GarrisonManager.prototype.numberOfGarrisonedSlots = function(holder)
{
	if (!this.holders.has(holder.id()))
...
PETRA_EXPERT.GarrisonManager.prototype.allowMelee = function(holder)
{
	if (!this.holders.has(holder.id()))
		return undefined;
...
PETRA_EXPERT.GarrisonManager.prototype.garrison = function(gameState, ent, holder, type)
{
	...
	ent.setMetadata(PlayerID, "subrole", PETRA_EXPERT.Worker.SUBROLE_GARRISONING);
 ...
 */
PETRA_EXPERT.GarrisonManager.prototype.leaveGarrison = function(ent)
{
...
PETRA_EXPERT.GarrisonManager.prototype.cancelGarrison = function(ent)
{
	ent.stopMoving();
	this.leaveGarrison(ent);
...
PETRA_EXPERT.GarrisonManager.prototype.keepGarrisoned = function(ent, holder, around)
{
	switch (ent.getMetadata(PlayerID, "garrisonType"))
	{
	case PETRA_EXPERT.GarrisonManager.TYPE_FORCE:           // force the ungarrisoning
		return false;
	case PETRA_EXPERT.GarrisonManager.TYPE_TRADE:		// trader garrisoned in ship
		return true;
	case PETRA_EXPERT.GarrisonManager.TYPE_PROTECTION:	// hurt unit for healing or infantry for defense
		...
		if (ent.attackTypes() && ent.attackTypes().indexOf("Melee") !== -1)
			return false;
		if (around.unit)
			return ent.hasClass("Support") || PETRA_EXPERT.isSiegeUnit(ent);	// only ranged siege here and below as melee siege already released above
		if (PETRA_EXPERT.isSiegeUnit(ent))
			return around.meleeSiege;
		return holder.buffHeal() && ent.needsHeal();
	case PETRA_EXPERT.GarrisonManager.TYPE_DECAY:
		return ent.captureStrength() && this.decayingStructures.has(holder.id());
	case PETRA_EXPERT.GarrisonManager.TYPE_EMERGENCY: // f.e. hero in regicide mode
		if (holder.buffHeal() && ent.isHealable() && ent.healthLevel() < this.Config.garrisonHealthLevel.high)
			return true;
	...
	default:
		...
		ent.setMetadata(PlayerID, "garrisonType", PETRA_EXPERT.GarrisonManager.TYPE_PROTECTION);
		return true;

	case PETRA_EXPERT.GarrisonManager.TYPE_SUPPORT:
		// 
		return around.unit || around.defenseStructure || around.meleeSiege || around.rangeSiege;
	}
};

/** Add this holder in the list managed by the garrisonManager */
PETRA_EXPERT.GarrisonManager.prototype.registerHolder = function(gameState, holder)
{
 ...
 */
PETRA_EXPERT.GarrisonManager.prototype.addDecayingStructure = function(gameState, entId, justCaptured)
{
...
PETRA_EXPERT.GarrisonManager.prototype.removeDecayingStructure = function(entId)
{
	if (!this.decayingStructures.has(entId))
		return;
...
PETRA_EXPERT.GarrisonManager.prototype.Serialize = function()
{
	return { "holders": this.holders, "decayingStructures": this.decayingStructures };
};

PETRA_EXPERT.GarrisonManager.prototype.Deserialize = function(data)
{
	for (let key in data)
		this[key] = data[key];
};
