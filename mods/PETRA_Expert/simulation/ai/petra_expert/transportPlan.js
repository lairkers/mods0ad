...
PETRA_EXPERT.TransportPlan = function(gameState, units, startIndex, endIndex, endPos, ship)
{
	...
	this.state = PETRA_EXPERT.TransportPlan.BOARDING;
	this.boardingPos = {};
 ...
 */
PETRA_EXPERT.TransportPlan.BOARDING = "boarding";
/**
 * We're moving ships and eventually unload units.
 */
PETRA_EXPERT.TransportPlan.SAILING = "sailing";

PETRA_EXPERT.TransportPlan.prototype.init = function(gameState)
{
...
PETRA_EXPERT.TransportPlan.prototype.countFreeSlots = function()
{
	let slots = 0;
...
PETRA_EXPERT.TransportPlan.prototype.countFreeSlotsOnShip = function(ship)
{
...
PETRA_EXPERT.TransportPlan.prototype.assignUnitToShip = function(gameState, ent)
{
...
PETRA_EXPERT.TransportPlan.prototype.assignShip = function(gameState)
{
	let pos;
...
PETRA_EXPERT.TransportPlan.prototype.addUnit = function(unit, endPos)
{
...
PETRA_EXPERT.TransportPlan.prototype.removeUnit = function(gameState, unit)
{
...
PETRA_EXPERT.TransportPlan.prototype.releaseShip = function(ship)
{
	if (ship.getMetadata(PlayerID, "transporter") != this.ID)
	{
		API3.warn(" Petra Expert: try removing a transporter ship with " + ship.getMetadata(PlayerID, "transporter") +
	...
	if (ship.getMetadata(PlayerID, "role") === PETRA_EXPERT.Worker.ROLE_SWITCH_TO_TRADER)
		ship.setMetadata(PlayerID, "role", PETRA_EXPERT.Worker.ROLE_TRADER);
};

PETRA_EXPERT.TransportPlan.prototype.releaseAll = function()
{
...
PETRA_EXPERT.TransportPlan.prototype.cancelTransport = function(gameState)
{
 ...
 */
PETRA_EXPERT.TransportPlan.prototype.update = function(gameState)
{
	if (this.state === PETRA_EXPERT.TransportPlan.BOARDING)
		this.onBoarding(gameState);
	else if (this.state === PETRA_EXPERT.TransportPlan.SAILING)
		this.onSailing(gameState);

	return this.units.length;
};

PETRA_EXPERT.TransportPlan.prototype.onBoarding = function(gameState)
{
	let ready = true;
	...
	this.state = PETRA_EXPERT.TransportPlan.SAILING;
	this.nTry = {};
	this.unloaded = [];
	this.recovered = [];
};
...
PETRA_EXPERT.TransportPlan.prototype.isOnBoard = function(ent)
{
	for (let ship of this.transportShips.values())
	{
...
PETRA_EXPERT.TransportPlan.prototype.getBoardingPos = function(gameState, ship, landIndex, seaIndex, destination, avoidEnnemy)
{
...
PETRA_EXPERT.TransportPlan.prototype.onSailing = function(gameState)
{
	...
	for (let entId of this.unloaded)
	{
		...
		else if (!ent.position())  // unloading failed
		{
			...
			if (ship)
			{
				...
				{
					API3.warn("Petra Expert transportPlan problem: unit not on ship without position ???");
					this.resetUnit(gameState, ent);
					ent.destroy();
				}
			}
			...
			{
				API3.warn("Petra Expert transportPlan problem: unit on ship, but no ship ???");
				this.resetUnit(gameState, ent);
				ent.destroy();
			}
		}
		else if (PETRA_EXPERT.getLandAccess(gameState, ent) != this.endIndex)
		{
...
PETRA_EXPERT.TransportPlan.prototype.resetUnit = function(gameState, ent)
{
	ent.setMetadata(PlayerID, "transport", undefined);
	ent.setMetadata(PlayerID, "onBoard", undefined);
	ent.setMetadata(PlayerID, "endPos", undefined);
...
PETRA_EXPERT.TransportPlan.prototype.Serialize = function()
{
	return {
		"ID": this.ID,
...
PETRA_EXPERT.TransportPlan.prototype.Deserialize = function(data)
{
	for (let key in data)
		this[key] = data[key];
