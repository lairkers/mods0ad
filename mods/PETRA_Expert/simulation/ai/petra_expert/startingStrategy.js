...
PETRA_EXPERT.HQ.prototype.gameAnalysis = function(gameState)
{
	...
	this.canExpand = this.Config.difficulty != PETRA_EXPERT.DIFFICULTY_SANDBOX;
 ...
 */
PETRA_EXPERT.HQ.prototype.assignStartingEntities = function(gameState)
{
	for (let ent of gameState.getOwnEntities().values())
	{
		...
		if (!pos)
		{
			...
			if (ent.isGarrisonHolder() && ent.garrisoned().length)
				API3.warn("Petra Expert warning: support for garrisoned units inside garrisoned holders not yet implemented");
			continue;
		}
 ...
 */
PETRA_EXPERT.HQ.prototype.regionAnalysis = function(gameState)
{
	...
	{
		API3.warn("Petra Expert error: it does not know how to interpret this map");
		return false;
	}
 ...
 */
PETRA_EXPERT.HQ.prototype.structureAnalysis = function(gameState)
{
 ...
 */
PETRA_EXPERT.HQ.prototype.buildFirstBase = function(gameState)
{
	...
	if (!total.canAfford(new API3.Resources(template.cost())))
	{
		...
		for (let ent of gameState.getOwnUnits().values())
		{
			...
			if (ent.isIdle())
				PETRA_EXPERT.gatherTreasure(gameState, ent);
	...
	for (let ent of gameState.getOwnUnits().values())
	{
		if (!ent.hasClass("Worker"))
			continue;
		if (PETRA_EXPERT.isFastMoving(ent))
			continue;
		let pos = ent.position();
		if (!pos)
		{
	...
	if (goal == "dock")
	{
		...
		gameState.ai.queues.dock.addPlan(new PETRA_EXPERT.ConstructionPlan(gameState, "structures/{civ}/dock", { "sea": sea, "proximity": startingPoint[imax].pos }));
	}
	else
		gameState.ai.queues.civilCentre.addPlan(new PETRA_EXPERT.ConstructionPlan(gameState, "structures/{civ}/civil_centre", { "base": -1, "resource": "wood", "proximity": startingPoint[imax].pos }));
};
 ...
 */
PETRA_EXPERT.HQ.prototype.dispatchUnits = function(gameState)
{
	...
	if (allycc.length)
	{
		...
		units.filter(API3.Filters.byClasses(["Infantry+Ranged"])).forEach(ent => {
			...
			{
				if (!cc.position() || PETRA_EXPERT.getLandAccess(gameState, cc) != access)
					continue;
				--num;
				--num1;
 ...
 */
PETRA_EXPERT.HQ.prototype.configFirstBase = function(gameState)
{
	if (!this.hasPotentialBase())
		return;
	...
	if (startingSize < 25000)
	{
		...
		this.Military.popForBarracks4 = Infinity;
		this.Military.popForBarracks5 = Infinity;
	}
	...
	if (!gameState.getOwnEntitiesByClass("DropsiteWood", true).hasEntities())
	{
		...
		if (newDP.quality > 40 && this.canBuild(gameState, newDP.templateName))
		{
			...
			gameState.ai.queues.dropsites.addPlan(new PETRA_EXPERT.ConstructionPlan(gameState, newDP.templateName, { "base": this.baseManagers()[0].ID }, newDP.pos));
		}
	}
	////////////////////////////////
	// let numWorkers = gameState.getOwnUnits().filter(API3.Filters.byClass("Worker")).length;
	// // immediatly build a wood dropsite if possible.
	// if (!gameState.getOwnEntitiesByClass("DropsiteMetal", true).hasEntities() && numWorkers > 50)
	// {
	// 	const newDP = this.baseManagers()[0].findBestDropsiteAndLocation(gameState, "metal");
	// 	if (newDP.quality > 40 && this.canBuild(gameState, newDP.templateName))
	// 	{
	// 		// if we start with enough workers, put our available resources in this first dropsite
	// 		// same thing if our pop exceed the allowed one, as we will need several houses
	// 		let numWorkers = gameState.getOwnUnits().filter(API3.Filters.byClass("Worker")).length;
	// 		if (numWorkers > 50 && newDP.quality > 60 ||
	// 			gameState.getPopulation() > gameState.getPopulationLimit() + 20)
	// 		{
	// 			const cost = new API3.Resources(gameState.getTemplate(newDP.templateName).cost());
	// 			gameState.ai.queueManager.setAccounts(gameState, cost, "dropsites");
	// 		}
	// 		gameState.ai.queueManager.changePriority("dropsites", 5 * this.Config.priorities.dropsites);
	// 		gameState.ai.queues.dropsites.addPlan(new PETRA_EXPERT.ConstructionPlan(gameState, newDP.templateName, { "base": this.baseManagers()[0].ID }, newDP.pos));
	// 	}
	// }
	// if (!gameState.getOwnEntitiesByClass("DropsiteStone", true).hasEntities() && numWorkers > 60)
	// {
	// 	const newDP = this.baseManagers()[0].findBestDropsiteAndLocation(gameState, "stone");
	// 	if (newDP.quality > 40 && this.canBuild(gameState, newDP.templateName))
	// 	{
	// 		// if we start with enough workers, put our available resources in this first dropsite
	// 		// same thing if our pop exceed the allowed one, as we will need several houses
	// 		let numWorkers = gameState.getOwnUnits().filter(API3.Filters.byClass("Worker")).length;
	// 		if (numWorkers > 60 && newDP.quality > 60 ||
	// 			gameState.getPopulation() > gameState.getPopulationLimit() + 20)
	// 		{
	// 			const cost = new API3.Resources(gameState.getTemplate(newDP.templateName).cost());
	// 			gameState.ai.queueManager.setAccounts(gameState, cost, "dropsites");
	...
	if (this.needCorral)
	{
		...
		if (!gameState.getOwnEntitiesByClass("Corral", true).hasEntities() && this.canBuild(gameState, template))
			gameState.ai.queues.corral.addPlan(new PETRA_EXPERT.ConstructionPlan(gameState, template, { "base": this.baseManagers()[0].ID }));
	}
};
