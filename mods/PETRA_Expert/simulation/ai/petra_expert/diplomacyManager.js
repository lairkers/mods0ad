 ...
 */
PETRA_EXPERT.DiplomacyManager = function(Config)
{
	this.Config = Config;
 ...
 */
PETRA_EXPERT.DiplomacyManager.prototype.init = function(gameState)
{
 ...
 */
PETRA_EXPERT.DiplomacyManager.prototype.tributes = function(gameState)
{
	...
	for (let i = 1; i < gameState.sharedScript.playersData.length; ++i)
	{
			...
			else if (this.Config.chat && availableResources[res] === 0 && allyResources[res] > totalResources[res] + 600)
			{
				...
				for (let k = 0; k < mostNeeded.length; ++k)
				{
					if (mostNeeded[k].type == res && mostNeeded[k].wanted > 0)
					{
						...
						PETRA_EXPERT.chatRequestTribute(gameState, res);
		...
		if (this.Config.chat)
			PETRA_EXPERT.chatSentTribute(gameState, i);
		Engine.PostCommand(PlayerID, { "type": "tribute", "player": i, "amounts": tribute });
	}
};

PETRA_EXPERT.DiplomacyManager.prototype.checkEvents = function(gameState, events)
{
	...
	for (let evt of events.DiplomacyChanged)
	{
		...
		else if (gameState.sharedScript.playersData[evt.player].isAlly[PlayerID] && gameState.isPlayerEnemy(evt.player))
		{
			...
			PETRA_EXPERT.chatAnswerRequestDiplomacy(gameState, evt.player, "ally", response);
		}
 ...
 */
PETRA_EXPERT.DiplomacyManager.prototype.lastManStandingCheck = function(gameState)
{
	...
	if (playerToTurnAgainst)
	{
		...
		{
			if (request.status === "waitingForTribute")
				PETRA_EXPERT.chatAnswerRequestDiplomacy(gameState, player, request.requestType, "decline");
 ...
 */
PETRA_EXPERT.DiplomacyManager.prototype.handleDiplomacyRequest = function(gameState, player, requestType)
{
	...
	PETRA_EXPERT.chatAnswerRequestDiplomacy(gameState, player, requestType, response, requiredTribute);
};

PETRA_EXPERT.DiplomacyManager.prototype.changePlayerDiplomacy = function(gameState, player, newDiplomaticStance)
{
	...
	if (this.Config.chat)
		PETRA_EXPERT.chatNewDiplomacy(gameState, player, newDiplomaticStance);
};

PETRA_EXPERT.DiplomacyManager.prototype.checkRequestedTributes = function(gameState)
{
	for (let [player, data] of this.receivedDiplomacyRequests)
		if (data.status === "waitingForTribute" && gameState.ai.elapsedTime > data.warnTime)
		{
			if (data.sentWarning)
			{
				...
				PETRA_EXPERT.chatAnswerRequestDiplomacy(gameState, player, data.requestType, "decline");
			}
			else
			{
				data.sentWarning = true;
				data.warnTime = gameState.ai.elapsedTime + 60;
				PETRA_EXPERT.chatAnswerRequestDiplomacy(gameState, player, data.requestType, "waitingForTribute", {
					"wanted": data.wanted,
					"type": data.type
				});
			}
 ...
 */
PETRA_EXPERT.DiplomacyManager.prototype.sendDiplomacyRequest = function(gameState)
{
	let player;
	let max = 0;
	for (let i = 1; i < gameState.sharedScript.playersData.length; ++i)
	...
	PETRA_EXPERT.chatNewRequestDiplomacy(gameState, player, requestType, "sendRequest");
};

PETRA_EXPERT.DiplomacyManager.prototype.checkSentDiplomacyRequests = function(gameState)
{
	for (let [player, data] of this.sentDiplomacyRequests)
		...
		{
			PETRA_EXPERT.chatNewRequestDiplomacy(gameState, player, data.requestType, "requestExpired");
			this.sentDiplomacyRequests.delete(player);
		}
};

PETRA_EXPERT.DiplomacyManager.prototype.update = function(gameState, events)
{
...
PETRA_EXPERT.DiplomacyManager.prototype.Serialize = function()
{
	return {
...
PETRA_EXPERT.DiplomacyManager.prototype.Deserialize = function(data)
{
	for (let key in data)
		this[key] = data[key];
};
